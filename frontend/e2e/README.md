...
| `e2e-web-full` | ubuntu-latest | 同上 | ⚠️ **best-effort**（gate：failed>2 红） | build + preview dist-web → `npx playwright test --grep "@web\b" --grep-invert "@web-smoke"` | `@web` ×18（全量除 smoke，含 FSA 授权流 + 下载面板 + 能力声明） |
...
> 并加失败计数 gate（阈值 8/2，均 < 实测测试总数 16/18，杜绝「全败仍全绿」）——测试失败仍出报告、
