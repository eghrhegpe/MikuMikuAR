// model-ops 拆分系列合并（focus/morph/physics/remove/vpd 5 文件 → 1）
// [2026-08] 同系列合并以省 isolate 单文件 import 成本（vitest.config 同款先例：
// model-detail-ui 3→1、library-core 6→1）。共享样板（vi.hoisted DOM 预建 +
// 7 条 vi.mock + model-ops-mocks 工厂）原在 5 文件重复 5 份，现收敛为一份。
// 各 describe 用例按原主题分区保留，行为不变。
