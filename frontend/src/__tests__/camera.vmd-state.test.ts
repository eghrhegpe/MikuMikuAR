...
describe('VMD', () => {
    it('hasCameraVmd false default', () => { expect(cam.hasCameraVmd()).toBe(false); });
    it('clearCameraVmd no throw', () => { expect(() => cam.clearCameraVmd()).not.toThrow(); });
    it('getCameraVmdName empty', () => { expect(cam.getCameraVmdName()).toBe(''); });
    it('getCameraVmdPath empty', () => { expect(cam.getCameraVmdPath()).toBe(''); });
    it('clearCameraVmd stays false', () => { cam.clearCameraVmd(); expect(cam.hasCameraVmd()).toBe(false); });
});
describe('ConcertPaused', () => {
    it('default false', () => { expect(cam.getConcertPaused()).toBe(false); });
    it('set true', () => { cam.setConcertPaused(true); expect(cam.getConcertPaused()).toBe(true); });
    it('toggle back', () => { cam.setConcertPaused(true); cam.setConcertPaused(false); expect(cam.getConcertPaused()).toBe(false); });
    it('false->false', () => { cam.setConcertPaused(false); expect(cam.getConcertPaused()).toBe(false); });
});
describe('CameraMode type', () => { it('valid', () => { expect(['orbit','freefly','surround','concert','oneshot','vmd']).toContain(cam.getCameraMode()); }); });
describe('P1', () => {
    it('LEGACY_MODE_MAP 8 keys', () => { expect(Object.keys(cam.LEGACY_MODE_MAP).sort()).toEqual(['ar','beatcut','concert','freefly','oneshot','orbit','surround','vmd']); });
    it('orbit → {orbit,none}', () => { expect(cam.LEGACY_MODE_MAP.orbit).toEqual({control:'orbit',behavior:'none'}); });
    it('surround→turntable concert→concert', () => { expect(cam.LEGACY_MODE_MAP.surround.behavior).toBe('turntable'); expect(cam.LEGACY_MODE_MAP.concert.behavior).toBe('concert'); });
    it('vmd/oneshot', () => { expect(cam.LEGACY_MODE_MAP.vmd).toEqual({control:'orbit',behavior:'scripted',scripted:'loop'}); expect(cam.LEGACY_MODE_MAP.oneshot).toEqual({control:'orbit',behavior:'scripted',scripted:'oneshot'}); });
    it('deriveLegacyMode roundtrip', () => { for(const[m,axes]of Object.entries(cam.LEGACY_MODE_MAP)){expect(cam.deriveLegacyMode(axes.control,axes.behavior,axes.scripted)).toBe(m);} });
    it('deriveLegacyMode beatcut', () => { expect(cam.deriveLegacyMode('orbit','beatcut')).toBe('beatcut'); });
});
describe('P2', () => {
    beforeEach(() => { cam.setAutoCameraEnabled(false); mockUiState.autoCameraEnabled = false; mockPBD.mockReturnValue(null); vi.clearAllMocks(); });
    it('default orbit/none/!auto', () => { expect(cam.getCameraControl()).toBe('orbit'); expect(cam.getCameraBehavior()).toBe('none'); expect(cam.isAutoCameraEnabled()).toBe(false); });
    it('enable → beatcut+subscribe', () => { const unsub=vi.fn();const d={onBeat:vi.fn(()=>unsub)}; cam.setAutoCameraEnabled(true,d); expect(cam.getCameraBehavior()).toBe('beatcut'); expect(d.onBeat).toHaveBeenCalledTimes(1); });
    it('disable → none+unsubscribe', () => { const unsub=vi.fn();const d={onBeat:vi.fn(()=>unsub)}; cam.setAutoCameraEnabled(true,d); cam.setAutoCameraEnabled(false); expect(cam.getCameraBehavior()).toBe('none'); expect(unsub).toHaveBeenCalledTimes(1); });
    it('fallback to procBeatDetector', () => { const unsub=vi.fn();const d={onBeat:vi.fn(()=>unsub)}; mockPBD.mockReturnValue(d); cam.setAutoCameraEnabled(true); expect(mockPBD).toHaveBeenCalled(); expect(d.onBeat).toHaveBeenCalledTimes(1); });
    it('restoreAutoCameraState re-subscribes', () => { const unsub=vi.fn();const d={onBeat:vi.fn(()=>unsub)}; mockPBD.mockReturnValue(d); mockUiState.autoCameraEnabled=true; mockUiState.autoCameraBeatsPerSwitch=4; cam.restoreAutoCameraState(); expect(cam.isAutoCameraEnabled()).toBe(true); expect(d.onBeat).toHaveBeenCalledTimes(1); });
});
