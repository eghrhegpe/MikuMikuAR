...
import { _catState, _matState, _matEnabled, setMatCatParams, setMatParams, resetSingleMatParams, _applyAll } from '../scene/scene';
...
function applyPerMatToBabylonMat(
    ...
    mat.diffuseColor.set(origDiffuse.r * params.diffuseMul, origDiffuse.g * params.diffuseMul, origDiffuse.b * params.diffuseMul);
    mat.specularColor.set(0.8 * params.specularMul, 0.8 * params.specularMul, 0.8 * params.specularMul);
...
describe('per-material params write through to Babylon material properties', () => {
    it('writes diffuseMul → diffuseColor.set with correct multiplier', () => {
        const mat = { diffuseColor: new SpyColor3(), specularColor: new SpyColor3(), specularPower: 50, ambientColor: new SpyColor3() };
