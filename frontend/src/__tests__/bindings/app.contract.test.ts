...
// 共享期望导出清单（单一来源）——此前在「function exports」与「method IDs」
// 两个 describe 各复制一份 151 项数组，增删函数需同步两处，漏改即契约覆盖
// 静默分裂。现收敛为单常量，两处 describe 引用同一来源。
...
describe('binding contract: interface shapes', () => {
    ...
    it('EnvState', () => {
        ...
        expect(f).toMatchObject({
            ...
            iblIntensity: expect.any(Number),
            groundVisibleEnabled: expect.any(Boolean),
            ...
            foamEnabled: expect.any(Boolean),
            foamThreshold: expect.any(Number),
            foamIntensity: expect.any(Number),
            foamOpacity: expect.any(Number),
            foamTransitionRange: expect.any(Number),
            foamColor: expect.any(Array),
            foamNoiseStrength: expect.any(Number),
            underwaterEnabled: expect.any(Boolean),
...
describe('binding contract: function exports', () => {
    const expectedFunctions = EXPECTED_FUNCTIONS;
