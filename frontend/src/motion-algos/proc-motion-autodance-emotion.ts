...
const MORPH_BLACKLIST_PATTERNS = [
    ...
    'blink',
    '眨眼',
    // 注意：'ウィンク'/'wink' 不在此黑名单——它们是合法情绪候选（EMOTION_CANDIDATES.wink），
    // 若同时列入黑名单会导致评分抵消（+10 匹配 -10 黑名单 = 0），情绪轮永远选不中 wink。
    // 注意：单字符平假名（'あ'/'い'/'う'/'え'/'お'）已从黑名单移除——
    // 它们是 '笑い'/'悲しみ' 等合法情绪 morph 的子串，会导致误杀。
    // 口型 morph 通常以完整名称匹配（如 'あ', 'い' 单独出现时），
    // 但 substring 匹配精度不足，宁可不拦截口型也不应误杀情绪。
    'a ',
    'i ',
    'u ',
    'e ',
