...
/** 导出 ground schema 供 menu-registry 静态分析（ADR-093 元测试）
 *  合并 6 个子 schema 数组，返回完整 ground 面板的 schema 树 */
export function getGroundSchema(): MenuNode[] {
    return [
        // ===== 地面预设（顶部 chips，一键应用）=====
        {
...
export function buildGroundLevel(): PopupLevel {
    return buildLevel(t('env.ground'), (c) => {
        return renderMenu(getGroundSchema(), c);
    });
}
