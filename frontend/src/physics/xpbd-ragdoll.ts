...
function _extractRelativeQuaternion(parentMat: Float32Array, childMat: Float32Array): Float32Array {
    ...
    const parentRot = _mat3ToQuaternion(
        parentMat[0],
        parentMat[1],
        parentMat[2],
        parentMat[4],
    ...
    const childRot = _mat3ToQuaternion(
        childMat[0],
        childMat[1],
        childMat[2],
        childMat[4],
    ...
    // 共轭 = 逆（单位四元数）
    parentInv[0] = -parentRot[0];
    parentInv[1] = -parentRot[1];
    parentInv[2] = -parentRot[2];
    parentInv[3] = parentRot[3];
 ...
 */
export const RAGDOLL_JOINT_GROUPS: Record<
    string,
    { keywords: string[]; params: RagdollJointParams }
> = {
    spine: {
        keywords: ['上半身', '下半身', '腰', 'spine', 'chest'],
        params: {
            compliance: 0,
            stiffness: 1,
            damping: 0.1,
            coneHalfAngle: Math.PI / 6,
            twistRange: [-Math.PI / 8, Math.PI / 8],
        },
    },
    shoulder: {
        keywords: ['肩', '腕', 'shoulder', 'arm'],
        params: {
            compliance: 0,
            stiffness: 1,
            damping: 0.05,
...
export function getJointParams(
    boneName: string,
    overrides?: Record<string, RagdollJointParams>
): RagdollJointParams {
    if (overrides?.[boneName]) {
        return overrides[boneName];
    }
    for (const g of Object.values(RAGDOLL_JOINT_GROUPS)) {
        if (g.keywords.some((kw) => boneName.toLowerCase().includes(kw.toLowerCase()))) {
            return g.params;
        }
    }
...
export function buildRagdoll(
    ...
    for (let i = 0; i < particles.length; i++) {
        ...
        if (parentIdx === -1) {
            continue;
        } // parent not in ragdoll (e.g., skipped)
