...
export function registerControlActions(): void {
  ...
  registerAction({
    ...
    label: '切换环境预设',
    domain: 'env',
    params: [{ name: 'preset', type: 'enum', enum: ['dawn', 'noon', 'sunset', 'night', 'overcast', 'neon'] }],
    execute: (p) => { applyEnvPreset(p.preset as string); },
  });
  ...
  registerAction({
    ...
    destructive: true,
    execute: async (p) => { loadLibraryModel(p.name as string); },
  });
  ...
  registerAction({
    ...
    destructive: true,
    execute: async (p) => { loadLibraryMotion(p.name as string); },
  });
