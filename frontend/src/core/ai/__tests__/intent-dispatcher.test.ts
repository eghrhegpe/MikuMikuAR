...
describe('parseActionFromLLM', () => {
    ...
    it('P1: 带命名空间前缀的 action 保留原样', () => {
        const r = parseActionFromLLM(
            JSON.stringify({ action: 'ai:control:setLightIntensity', params: { dirIntensity: 0.5 } })
        );
...
describe('executeAction', () => {
    ...
    it('委托 executeActionById 执行', async () => {
        ...
        expect(executeActionById).toHaveBeenCalledWith('ai:control:setCameraMode', { mode: 'orbit' });
    });
});