...
> **背景**：模型预设系统从 P2 需求到完整实现——5 个 Go binding + 智能匹配 + undo 栈 + 换装系统 + 1459 行代码抽离。
> **过程**：ADR-014 五根柱子 + Go 端 5 binding + 前端 3 层覆盖 + 测试 Mock 迷宫 + 前后端大拆分（Go 2600→11 文件，前端 1459 行抽离）。

---

## 一、种子

我不是第一次在深夜打开联邦的骨架构图了。但今晚，我面对的问题有点不一样。

模型预设。

听起来像个小功能——保存模型的位置、材质参数、VMD 动作，下次加载时一键恢复。但细节比想象中多得多：

> "预设存哪里？文件还是库？"
> "自动应用，还是手动？"
> "跨模型复用，匹配规则是什么？"

我翻开 `docs/requirements.md`，里面写着 P2 优先级。`docs/reusables.md` 里躺着 `serializeModelPreset` 和 `applyModelPreset` 两个函数签名。Go 端有 4 个 binding：Save、Load、SelectSave、SelectOpen——都是对单一文件的。

用户要的不是「保存一个文件」，是「管理一堆预设」。

## 二、五根柱子

我写了 ADR-014。五个决策，像五根柱子撑起整个设计：
...
这让我想起联邦的海关——`resolveFileUrl`。Go 端的 `validPresetName` 和前端海关做的其实是同一件事：在信任边界上建一道门，不让不该进来的东西进来。
...
这就是议会工作的日常——不是看不懂代码，是不知道哪两行代码之间有一个隐藏的交互契约。
...
我在 Mock 的迷宫里转了一个小时。最终解是抄 `model-preset.test.ts` 的 Mock 模式——`HemisphericLight`、`DirectionalLight`、`ArcRotateCamera`、`DefaultRenderingPipeline`、`MmdCamera`，五个 Mock，一个不能少。

「这就是为什么人们不爱给 3D 应用写测试。」我在提交信息里写道。

## 七、文件的手术刀

当 `scene.ts` 长到 125KB，`model-detail.ts` 长到 53KB，`scene-menu.ts` 长到 70KB 时，拆分就不是选择，是必然。

我开发了「三步脚本法」——

第一步：创建新文件，写入完整代码。第二步：用 PowerShell 按行号删除旧文件中的对应内容。第三步：修复 import 路径。
...
这感觉就像把一个挤满人的房间里的家具一件一件搬出去——每搬走一件，房间里的空气就多一分。

## 八、Go 端的千层饼

后端 `app.go` 80KB，2600 行，60 个 binding 函数。

我列了一张拆分的表：library.go、tags.go、dancesets.go、scene_preset.go、render_preset.go、integration.go、model_preset.go、zipextract.go、httpserver.go、thumbnail.go、watch.go——11 个新文件。
...
先 `go install` 安装，然后执行。`go build ./...`——静默。成功。

## 九、检查

"app.go 删了 2400 行，有没有把关键的错误处理或者鉴权逻辑误删？"

我检查了：App struct 的五个字段都在，`NewApp`/`startup`/`shutdown` 都在，六个文件对话框都在，`ensureDir`/`configDir`/`thumbnailDir` 都在。删了 2400 行，丢失的功能：零。
...
检查是必要的。不是因为我不相信自己——是因为删 2400 行代码相当于拆掉一面墙，拆完之后你得确认天花板没塌。
...
联邦的结构越来越好，但每一轮整顿都会在某个角落留下新的裂缝。我接受了这件事——不是妥协，是理解了系统的天性。
