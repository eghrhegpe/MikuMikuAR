...
> 第三卷 · 铁腕整顿（第二波）
>
> 对应真实事件：Ctrl+1~5 声明式重构与 ARIA 无障碍落地方案 — 接近 12 轮 AI 交互的全量硬编码清除、术语本地化（Glossary）、data-overlay 动态查询、SlideMenu 键盘导航、音乐控件/循环按钮/模型详情 UI 大修

---

## 零、迟到的修复

Sisyphus 把「错位的五键」那篇文档贴在公告栏的时候，写过一句话——「会修的。」

那是上一轮的事了。

这一轮他回来了，带着方案回来的。

他不打算在旧的 `switch` 语句上加两个 `case`。那样太无聊了。他在过去几十轮交互里养成了一个习惯：**在一个硬编码的地方发现空槽，那说明整个方案都应该换成声明式的。**

Ctrl+1~5 的问题不是少了两个 `case`。问题是五个按钮对应五条键盘通道，但代码用了一段硬编码的 `switch (num)` 来路由。加 `case 3` 和 `case 5` 只是把五条路修成五条——但下次加一个按钮呢？加 Ctrl+6、Ctrl+7？再回去改 `switch`？

「不修了，重建。」

Sisyphus 在 `main.ts` 里画了一条线，线的左边是旧的 `triggerNavButton`，右边是新的 `navActions`：
...
但他没停下来。他发现 `index.html` 里五个按钮都有 `data-shortcut` 属性，从 `1` 到 `5`，整整齐齐。但坏就坏在整整齐齐——`data-shortcut` 在那里挂了不知道多久，却没有任何代码读它。`navButtonLabels` 是一个独立的 `Record<number, string>`，`data-shortcut` 是另一个，快捷键路由 `switch` 是第三个。三本账各记各的，对不上活该。
...
但 Sisyphus 知道，真正让他不安的不是那两条断掉的路——而是这条路居然花了五章小说才修好。从「裂隙初现」到「错位的五键」，中间隔了三个卷章、十几个城邦的故事。快捷键修好只用了十五分钟，理解它为什么坏了用了四十章。

这就是聚合者的代价——修一个 bug 的时间永远远小于找到它根因的时间。
