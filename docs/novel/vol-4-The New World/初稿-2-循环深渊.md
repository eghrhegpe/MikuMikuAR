...
> 第三卷 · 铁腕整顿 · 第18章
> 对应真实事件：library.ts 拆分解耦后的循环依赖调试 + 卡片化 UI 批量改造
...
循环依赖修完，Sisyphus 回头做他真正想做的事——把所有菜单都变成卡片。

在「重构风暴」那一章里，场景菜单已经是卡片了。但其它菜单还是老样子：扁平的 `items` 数组，一条接一条地排列，没有分组，没有视觉节奏。

```
「对齐，」用户说。
```

Sisyphus 打开 motion-popup 的根菜单。一个可滚动的列表，模型行和功能行混在一起，中间插着一条分隔线。这是 2024 年的 UI 风格——功能性上没问题，视觉上让人提不起兴趣。
...
同样的信息量，但视觉上清晰了一倍。卡片的边界代替了分隔线，模型和操作不再是一个列表里的两个条目——它们是两个独立的功能区，各自有各自的语义。

然后是设置菜单。五个设置项（显示、界面、下载、系统、软件管理）曾经挤在一个列表里，现在变成一张卡片：
...
接下来是子菜单。系统预设子菜单、显示设置子菜单、外部库管理子菜单、软件管理子菜单、动作绑定子菜单、最近打开——Sisyphus 一个一个地翻修。每个子菜单的改造只有十几行代码，但十几个子菜单加起来就是一次系统性的视觉统一。

有些转换很顺利。比如显示设置：原来就是几个 `items` 条目，每个条目一个图标和一个勾选状态。用 `renderCustom` + `lcard` 包一下，三分钟搞定。

有些转换踩了坑。软件管理子菜单有一个 bug：`renderCustom` 覆盖了 `items`，导致操作按钮完全不可见。修复办法是把 `items` 全部搬到 `renderCustom` 中，纯手工 DOM 构建。

「每张卡片都是独立的，」Sisyphus 总结规则，「卡片内部的行元素透明，靠 hover 变亮来反馈。卡片之间 6px 间隙。没有分隔线。」

---

## 六、文件浏览器的最后一公里

最难改造的是 `buildLevel`——文件浏览器。它不只是显示文件和目录，它还承载了几种特殊的文件夹（最近打开、搜索、标签筛选），每种特殊文件夹有不同的渲染方式。

原始代码用一个 `renderCustom` 回调在内容渲染后手动插入特殊行：
...
Sisyphus 把文件内容包进 `lcard`，同时在参数中增加了 `targetStack` 和 `extraFolders`——解决两个问题：
1. 从不同弹窗（模型库 vs 动作弹窗）打开文件浏览器时，导航栈不一样
2. 不同场景下需要显示不同的额外文件夹

改完的那一刻，他看着最终效果——模型库弹窗的每一层都有卡片包裹，文件内容在卡片里排列整齐，特殊文件夹在卡片顶部以 `slideRow` 的方式呈现——他感到一种近似偏执的满足。

「这艘船的所有木板，现在都是同一种木头了，」他想。
