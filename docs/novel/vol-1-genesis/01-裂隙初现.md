...
**第一卷 · 创世纪 | 对应真实事件**：Bug #3 — VMD 加载失败，缺失 `mmdWasmRuntimeModelAnimation` 副作用 import；babylon-mmd 第一城邦入伙
...
**缝**：babylon-mmd 的能力分正门与侧门。正门的靠 import 显式带入，侧门的靠副作用隐式挂载。聚合者不提前问"有几扇门"，就会在"is not a function"里栽跟头。

**教训**：收编一个城邦前，先问清它的能力都从哪扇门来。
