...
这张表照搬了 `LightState` 的模式——那边也是一堆字段，一个 `getLightState` 读，一个 `setLightState` 写。桌面壳照着写了 `getRenderState` 和 `setRenderState`。

"你又在复制模式。"`MenuStack` 说。

"模式复用不是复制。"桌面壳说，"用户学会了调灯光，就自然会调渲染——因为交互一样。滑块拖一下，状态变一下，`triggerAutoSave` 存一下。零学习成本。"

`getRenderState` 从 pipeline、scene、activeCamera 读当前值。`setRenderState` 接受 `Partial<RenderState>`——传什么改什么，不传的不动。这让预设系统可以做"只改这几个参数"的部分应用。
...
用户自己的预设不能只存在内存里。关了应用就没了。

桌面壳找到 `app.go`——那个威严的总管，腰间挂着三十五把钥匙。
...
`app.go` 点头。它在 `Config` 结构体里加了一个字段：
...
然后三个 binding，照着 `SetDisplayNamePriority` 的模式写——`updateConfig` 加锁、读、改、写，一气呵成：
...
"为什么 Params 是 `map[string]interface{}` 而不是 `RenderState`？"`app.go` 问。

"因为 Go 不知道前端的 `RenderState` 长什么样。"桌面壳说，"而且以后可能加材质参数。用 map 留口子，不锁死结构。"

`app.go` 沉默了一拍。它不喜欢弱类型——但承认这是合理的妥协。它给三把钥匙挂上了腰间。

Wails 的绑定生成器跑了一遍。`models.ts` 多了 `RenderPreset` 类，`App.d.ts` 和 `App.js` 多了三个函数。前端的 `scene-menu.ts` import 了它们。
