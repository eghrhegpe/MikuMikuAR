...
> **自动生成**（2026-08-17）— 由 `scripts/gen-funcmap.mjs` 生成。
...
| `Prompt2Options()` | `core/dialog:313` | — |
| `disposeOverlay2()` | `core/dialog:389` | 移除 showPrompt2 创建的 overlay2 DOM（供 HMR 清理入口调用）。 |
| `showConfirm()` | `core/dialog:274` | Show a confirmation dialog. |
| `showPrompt()` | `core/dialog:290` | Show a prompt dialog. |
| `showPrompt2()` | `core/dialog:419` | 双字段输入对话框。返回 [value1, value2] 或 null（取消）。 |
...
| `animateCameraVmd()` | `scene/camera/camera-vmd:100` | Animate the VMD camera to a given 30fps frame time. |
| `clearCameraVmd()` | `scene/camera/camera-vmd:77` | — |
| `createVmdCamera()` | `scene/camera/camera-vmd:107` | 创建 VMD 相机（若已存在且未销毁则复用）。供 camera.ts switchCameraMode 在 vmd 分支使用。 |
| `hasCameraAnimationHandle()` | `scene/camera/camera-vmd:126` | VMD 相机动画句柄是否就绪（switchCameraMode 在 vmd 分支前置检查）。 |
...
| `CameraPreset()` | `scene/camera/camera:830` | — |
...
| `ScriptedSubMode()` | `scene/camera/camera:830` | — |
...
| `animateCameraVmd()` | `scene/camera/camera:862` | — |
| `autoFrame()` | `scene/camera/camera:578` | Auto-frame the camera to centre on a bounding box. |
| `clearCameraVmd()` | `scene/camera/camera:862` | — |
| `defaultCameraPreset()` | `scene/camera/camera:861` | — |
| `deriveLegacyMode()` | `scene/camera/camera:128` | ADR-100 §6.2 — 双轴 → 旧模式反查（getCameraState 降级双写 / shim 内部路由）。 |
| `disposeCameraSystem()` | `scene/camera/camera:799` | 顶层销毁相机系统（HMR / 页面卸载 / scene 销毁时调用）。幂等。 |
| `getAutoCameraBeatsPerSwitch()` | `scene/camera/camera:871` | — |
| `getBoneLockDamping()` | `scene/camera/camera:864` | — |
| `getCameraBehavior()` | `scene/camera/camera:841` | — |
| `getCameraControl()` | `scene/camera/camera:841` | — |
| `getCameraMode()` | `scene/camera/camera:841` | — |
| `getCameraState()` | `scene/camera/camera:624` | — |
| `getCameraVmdName()` | `scene/camera/camera:841` | — |
| `getCameraVmdPath()` | `scene/camera/camera:841` | — |
| `getConcertParams()` | `scene/camera/camera:841` | — |
| `getConcertPaused()` | `scene/camera/camera:841` | — |
| `getCurrentCamera()` | `scene/camera/camera:841` | — |
| `getFocusedModelBoneNames()` | `scene/camera/camera:864` | — |
| `getFov()` | `scene/camera/camera:841` | — |
| `getFreeflyParams()` | `scene/camera/camera:841` | — |
| `getOrbitBoneLock()` | `scene/camera/camera:864` | — |
| `getOrbitParams()` | `scene/camera/camera:841` | — |
| `getScriptedSubMode()` | `scene/camera/camera:841` | — |
| `getSurroundParams()` | `scene/camera/camera:841` | — |
| `getSurroundPaused()` | `scene/camera/camera:841` | — |
| `hasCameraVmd()` | `scene/camera/camera:841` | — |
| `initCameraSystem()` | `scene/camera/camera:352` | Initialise the camera system and create the default Orbit camera. |
| `isAutoCameraEnabled()` | `scene/camera/camera:878` | — |
| `loadCameraVmd()` | `scene/camera/camera:862` | — |
| `logCameraAlpha()` | `scene/camera/camera:188` | Log current camera alpha for diagnostics. |
| `refreshCameraUserSettings()` | `scene/camera/camera:863` | — |
| `restoreAutoCameraState()` | `scene/camera/camera:871` | — |
| `setAutoCameraBeatsPerSwitch()` | `scene/camera/camera:871` | — |
| `setAutoCameraEnabled()` | `scene/camera/camera:878` | — |
| `setBoneLockDamping()` | `scene/camera/camera:864` | — |
...
| `setCameraPreset()` | `scene/camera/camera:861` | — |
| `setCameraState()` | `scene/camera/camera:654` | — |
| `setConcertParams()` | `scene/camera/camera:210` | — |
| `setConcertPaused()` | `scene/camera/camera:841` | — |
| `setFov()` | `scene/camera/camera:342` | — |
| `setFreeflyParams()` | `scene/camera/camera:197` | — |
| `setOrbitBoneLock()` | `scene/camera/camera:864` | — |
| `setOrbitParams()` | `scene/camera/camera:170` | — |
| `setSurroundParams()` | `scene/camera/camera:214` | — |
| `setSurroundPaused()` | `scene/camera/camera:841` | — |
...
| `applyEnvPresetByCategory()` | `scene/env/env-time-of-day:372` | [adr-120] 按类别应用用户自定义预设。 |
...
| `applyMatState()` | `scene/manager/material:1025` | — |
| `applyUnlitFallback()` | `scene/manager/material:758` | 光照兜底预设：让模型呈现"伪 unlit"状态，不依赖方向光即可正常显示。 |
| `disposeModelMaterialState()` | `scene/manager/material:874` | 清理指定模型的全部材质状态（分类 + 逐材质 + 启用标记）。 |
| `getMatCatGroups()` | `scene/manager/material:687` | — |
| `getMatCatParams()` | `scene/manager/material:707` | — |
| `getMatDetailList()` | `scene/manager/material:788` | — |
| `getMatParams()` | `scene/manager/material:817` | — |
| `getMatState()` | `scene/manager/material:955` | — |
...
| `isMatCategoryAllEnabled()` | `scene/manager/material:883` | 检查指定分类的全部材质是否都已启用。 |
| `isMatEnabled()` | `scene/manager/material:658` | — |
| `isPbrMaterial()` | `scene/manager/material:452` | — |
| `resetMatCatParams()` | `scene/manager/material:734` | — |
| `resetPerMaterialParams()` | `scene/manager/material:949` | 重置所有逐材质覆盖（per-material），保留分类调整（皮肤/头发等）。 |
| `resetSingleMatParams()` | `scene/manager/material:855` | — |
| `setMatCatParams()` | `scene/manager/material:719` | — |
| `setMatCategoryEnabled()` | `scene/manager/material:911` | 按分类批量切换材质可见性。 |
| `setMatEnabled()` | `scene/manager/material:662` | — |
| `setMatParams()` | `scene/manager/material:827` | — |
...
| `_applyLipSync()` | `scene/motion/perception-lipsync:57` | — |
| `_disposeLipSyncRuntime()` | `scene/motion/perception-lipsync:53` | 释放指定模型的 lip-sync 运行时（模型移除时调用，防 Map 泄漏） |
...
| `_gazeLog()` | `scene/motion/perception-shared:468` | — |
| `_incGazeLogFrame()` | `scene/motion/perception-shared:465` | — |
...
| `_qAngleDeg()` | `scene/motion/perception-shared:477` | 两四元数夹角（度） |
...
| `feetDebug()` | `scene/motion/perception-shared:486` | — |
...
| `__testOnlyGetContext()` | `scene/motion/perception:841` | 测试用：获取指定模型的 context（含 lastOffsets） |
| `_clampEyeGazeTarget()` | `scene/motion/perception:59` | — |
| `_clampHeadGazeTarget()` | `scene/motion/perception:59` | — |
| `_getGazeResetTick()` | `scene/motion/perception:400` | 获取 gaze 重置计数（供测试验证调用时机） |
| `_isWasmRuntime()` | `scene/motion/perception:59` | — |
| `_propagateChildrenWasm()` | `scene/motion/perception:59` | — |
| `_resetGazeState()` | `scene/motion/perception:405` | 重置 gaze 增量状态（清理跨帧缓存，避免切换/开关后出现跳跃） |
| `_writeMatToBuffer()` | `scene/motion/perception:59` | — |
| `activatePerception()` | `scene/motion/perception:328` | 激活感知层（呼吸/眨眼/gaze） |
| `applyGazeWasm()` | `scene/motion/perception:59` | — |
| `deactivatePerception()` | `scene/motion/perception:414` | 注销感知层 |
| `disableAllPerception()` | `scene/motion/perception:753` | 全员关闭感知层（仅焦点 + pinned 保留） |
| `enableAllPerception()` | `scene/motion/perception:722` | 全员激活感知层（受 tier 限制） |
| `getPerceptionPerfManualTier()` | `scene/motion/perception:792` | [doc:adr-164] 获取手动档位设置（'auto' 表示自动降级模式） |
| `getPerceptionPerfTier()` | `scene/motion/perception:787` | 获取当前性能档位 |
| `getPerceptionState()` | `scene/motion/perception:449` | 获取感知状态（焦点 context 状态，兼容旧 API） |
| `getPerceptionStateFor()` | `scene/motion/perception:707` | 获取感知状态（场景级单例；参数对所有模型一致，modelId 参数保留仅为兼容旧调用） |
| `getPinnedModelIds()` | `scene/motion/perception:700` | 获取当前 pinned 模型 ID 列表 |
| `isAllPerceptionEnabled()` | `scene/motion/perception:803` | [doc:adr-164] 获取全员感知开关状态 |
| `onPerceptionModelRemoved()` | `scene/motion/perception:846` | 兼容接口：模型移除时清理（供 proc-motion-bridge.ts 调用） |
| `pinPerception()` | `scene/motion/perception:652` | [doc:adr-164] pin 模型感知（原 ≤5 上限已移除，全员感知由 tier 控制）。 |
| `setAllPerceptionEnabled()` | `scene/motion/perception:808` | [doc:adr-164] 设置全员感知开关状态 |
| `setBalanceSwayAmplitude()` | `scene/motion/perception:537` | 设置重心微动振幅（全局乘数，钳制 0–2.0） |
| `setBalanceSwayEnabled()` | `scene/motion/perception:525` | 设置重心微动开关（[doc:adr-079] Phase 2） |
| `setBalanceSwayPeriod()` | `scene/motion/perception:531` | 设置重心微动周期（秒，钳制 0.5–5.0） |
| `setBlinkAmplitude()` | `scene/motion/perception:593` | 设置眨眼幅度（0–1，钳制） |
| `setBlinkEnabled()` | `scene/motion/perception:499` | 设置眨眼开关 |
| `setBlinkFrequency()` | `scene/motion/perception:587` | 设置眨眼频率（Hz，钳制 0.05–0.5） |
| `setBreathAmplitude()` | `scene/motion/perception:581` | 设置呼吸幅度（弧度，钳制 0–0.05） |
| `setBreathEnabled()` | `scene/motion/perception:493` | 设置呼吸开关 |
| `setBreathFrequency()` | `scene/motion/perception:575` | 设置呼吸频率（Hz，钳制 0.1–1.0） |
| `setEmotion()` | `scene/motion/perception:543` | 设置情绪类型 |
| `setEyeGazeMaxPitch()` | `scene/motion/perception:620` | 设置眼部跟随最大俯仰角（度，钳制 0–15） |
| `setEyeGazeMaxYaw()` | `scene/motion/perception:613` | 设置眼部跟随最大偏航角（度，钳制 0–15） |
| `setEyeGazeSmooth()` | `scene/motion/perception:627` | 设置眼部跟随平滑度（0–1） |
| `setEyeTrackingEnabled()` | `scene/motion/perception:512` | 设置眼部跟随开关 |
| `setGazeConfig()` | `scene/motion/perception:821` | 兼容接口：设置 gaze 配置（供 proc-motion-bridge.ts 调用） |
| `setHeadGazeMaxPitch()` | `scene/motion/perception:606` | 设置头部跟随最大俯仰角（度，钳制 0–90） |
| `setHeadGazeMaxYaw()` | `scene/motion/perception:599` | 设置头部跟随最大偏航角（度，钳制 0–90） |
| `setHeadTrackingEnabled()` | `scene/motion/perception:505` | 设置头部跟随开关 |
| `setLipSyncEnabled()` | `scene/motion/perception:549` | 设置 lip-sync 开关 |
| `setLipSyncIntensity()` | `scene/motion/perception:561` | 设置 lip-sync 强度（钳制 0..1） |
| `setLipSyncMultiMorphEnabled()` | `scene/motion/perception:567` | 设置多口型 morph 开关 |
| `setLipSyncSensitivity()` | `scene/motion/perception:555` | 设置 lip-sync 灵敏度（钳制 0..1） |
| `setMicroExpressionEnabled()` | `scene/motion/perception:519` | 设置微表情开关 |
| `setPerceptionPerfTier()` | `scene/motion/perception:797` | 手动设置性能档位（auto/high/medium/low） |
...
| `AttachmentAnchors()` | `scene/physics/physics-bridge:75` | — |
| `AttachmentFit()` | `scene/physics/physics-bridge:80` | — |
| `FrameUpdateFn()` | `scene/physics/physics-bridge:130` | — |
| `PerFrameUpdateRegistry()` | `scene/physics/physics-bridge:137` | 单一 onBeforeRenderObservable 调度多个按 key 注册的每帧回调。 |
...
| `getBoneWorldPosition()` | `scene/physics/physics-bridge:60` | 从骨骼局部矩阵提取位置（米，场景单位）。 |
...
| `buildTagDetailLevel()` | `menus/library-actions:728` | — |
| `buildTagsOverviewLevel()` | `menus/library-actions:728` | — |
| `findLibraryModelByName()` | `menus/library-actions:728` | 按名称模糊搜索模型（纯查询，不触发加载）。供 ADR-155/197 NL 控制 resolve 使用，避免 resolve 阶段误触发真实加载。 |
| `findLibraryMotionByName()` | `menus/library-actions:728` | 按名称模糊搜索 VMD 动作（纯查询，不触发替换）。 |
| `highlightRow()` | `menus/library-actions:728` | — |
| `importFile()` | `menus/library-actions:683` | — |
| `importFileByPath()` | `menus/library-actions:638` | — |
| `onModelRowClick()` | `menus/library-actions:728` | — |
...
| `buildLevel()` | `menus/library-core:859` | — |
| `buildModelFormationLevel()` | `menus/library-core:903` | — |
| `buildModelRootItems()` | `menus/library-core:938` | — |
...
| `importFile()` | `menus/library-core:1051` | — |
| `initLibrary()` | `menus/library-core:1052` | — |
...
| `prepareModelRestore()` | `menus/library-core:1051` | — |
| `refreshLibrary()` | `menus/library-core:1052` | — |
| `refreshModelRoot()` | `menus/library-core:1029` | 就地更新模型库根层级数据（刷新已注册的角色列表）。 |
| `reloadConfig()` | `menus/library-core:1052` | — |
| `rescanAndSync()` | `menus/library-core:1052` | — |
| `resolveDisplayBrowseDir()` | `menus/library-core:140` | [修复] 解析模型在资源库中的"显示目录"——即用户点击该模型时实际看到的层级。 |
| `selectOverridePath()` | `menus/library-core:1052` | — |
| `selectResourceRoot()` | `menus/library-core:1052` | — |
| `setResourceViewMode()` | `menus/library-core:50` | — |
| `showModelPopup()` | `menus/library-core:1050` | — |
| `splitSubdirSegments()` | `menus/library-core:73` | — |
| `switchStorageMode()` | `menus/library-core:1052` | — |
