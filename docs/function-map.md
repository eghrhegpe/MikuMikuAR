...
| `_resetActionRegistry()` | `core/action-registry:102` | 清空注册表（测试/重置用） |
| `_setStrictMode()` | `core/action-registry:107` | 设置严格模式 |
| `getAction()` | `core/action-registry:84` | 按 id 获取动作定义 |
| `listActions()` | `core/action-registry:89` | 列出全部或指定域的动作 |
| `registerAction()` | `core/action-registry:63` | 注册一条动作。遇重复 id 时 console.warn + 覆盖（默认）。 |
| `registerActions()` | `core/action-registry:79` | 批量注册 |
| `unregisterAction()` | `core/action-registry:97` | 按 id 撤销注册 |
...
| `BrowseOutcome()` | `core/types:382` | — |
| `CameraBehavior()` | `core/types:592` | ADR-100 轴 B — 运动行为：相机如何自动运动，仅当控制轴为 `orbit`(ArcRotate) 时生效。 |
| `CameraControl()` | `core/types:580` | ADR-100 轴 A — 控制方案：决定相机类 + 输入方式。 |
| `CameraMode()` | `core/types:571` | 保留为兼容别名（存档 / 旧调用点），新代码请用 {@link CameraControl} × {@link CameraBehavior}。 |
| `DisplayNamePriority()` | `core/types:562` | — |
| `EnvState()` | `core/types:534` | 从 schema 派生 EnvState interface（-readonly 保证可写）。[doc:adr-137] |
| `FeetState()` | `core/types:85` | [doc:adr-085] 脚部地面跟随（按模型）状态 |
| `LibraryModel()` | `core/types:309` | — |
| `LibrarySortMode()` | `core/types:601` | — |
| `MmdRuntimeBoneExtended()` | `core/types:542` | — |
| `MmdStandardMaterial()` | `core/types:610` | MmdStandardMaterial 扩展 — 用于材质系统和换装系统共享的类型定义 |
| `ModelInstance()` | `core/types:182` | — |
| `ModelKind()` | `core/types:107` | — |
| `ModelMotionSlots()` | `core/types:159` | [doc:adr-167] 单槽位：overlay 槽位已移除（ADR-144 废弃） |
| `MotionModuleState()` | `core/types:40` | [doc:adr-116] 模块语义状态（per-motion，随动作走） |
| `MotionPreset()` | `core/types:53` | [doc:adr-145] 动作预设 DTO |
| `MotionSlotConfig()` | `core/types:146` | 单个槽位的配置 |
| `MotionSource()` | `core/types:125` | 用户选择的「原始动作来源类型」——仅描述意图来源性质，不描述广播后的运行时产物。 |
| `OutfitFile()` | `core/types:302` | — |
| `OutfitSlot()` | `core/types:281` | — |
| `OutfitVariant()` | `core/types:291` | — |
| `OverridePaths()` | `core/types:551` | — |
| `ParamValue()` | `core/types:37` | [doc:adr-116] 动作覆盖模块语义参数值 |
| `PendingVmd()` | `core/types:549` | — |
| `PhysicsCategory()` | `core/types:564` | — |
| `PopupLevel()` | `core/types:394` | — |
| `PopupRow()` | `core/types:324` | — |
| `PresetModuleState()` | `core/types:47` | [doc:adr-145] 单模块在预设中的状态快照 |
| `ProcMotionConfig()` | `core/types:70` | [doc:adr-XX] 程序化动作配置（per-motion，随动作走） 参数存 SceneMotionIntent.procMotion（多角色共享）， 启用/分配权在每角色 |
| `ProcPreset()` | `core/types:74` | [audit] 程序化动作自定义预设（per-model 参数快照，仿 MotionPreset 模式）。 |
| `RecentMotion()` | `core/types:603` | — |
| `RuntimeModel()` | `core/types:170` | IMmdModel 接口不含 setRuntimeAnimation / createRuntimeAnimation （这两个方法在 MmdModel 和 MmdWasmMode |
| `SceneMotionIntent()` | `core/types:128` | 场景级动作意图（「场上在跳什么」） |
| `ScriptedSubMode()` | `core/types:599` | ADR-100 §6.4 — `scripted` 行为的子模式。 |
...
| `CameraBehavior()` | `scene/camera/camera-state:24` | ADR-100 轴 B — 运动行为（仅对 orbit/ArcRotate 生效，初版互斥）。双写于 `core/types.ts`。 |
| `CameraControl()` | `scene/camera/camera-state:21` | ADR-100 轴 A — 控制方案（相机类 + 输入）。双写于 `core/types.ts`。 |
| `CameraMode()` | `scene/camera/camera-state:17` | 新代码请用 {@link CameraControl} × {@link CameraBehavior}。双写于 `core/types.ts`。 |
| `CameraPreset()` | `scene/camera/camera-state:65` | Per-mode parameter bundle, persisted with scene files. |
| `ConcertParams()` | `scene/camera/camera-state:54` | Concert (fan-cam) camera parameters — limited horizontal sweep + sinusoidal vertical bob. |
| `FreeflyParams()` | `scene/camera/camera-state:41` | Freefly camera parameters. |
| `OrbitParams()` | `scene/camera/camera-state:34` | Orbit camera parameters. |
| `ScriptedSubMode()` | `scene/camera/camera-state:27` | ADR-100 §6.4 — scripted 行为子态。 |
| `SurroundParams()` | `scene/camera/camera-state:47` | Surround (turntable) camera parameters — automatic full-circle orbit around target. |
| `clearCameraVmdState()` | `scene/camera/camera-state:243` | — |
| `defaultCameraPreset()` | `scene/camera/camera-state:93` | — |
| `getAutoCameraBeatCount()` | `scene/camera/camera-state:258` | — |
| `getAutoCameraPresetIdx()` | `scene/camera/camera-state:266` | — |
| `getCameraBehavior()` | `scene/camera/camera-state:152` | — |
| `getCameraCanvas()` | `scene/camera/camera-state:301` | — |
| `getCameraControl()` | `scene/camera/camera-state:148` | — |
| `getCameraMode()` | `scene/camera/camera-state:144` | — |
| `getCameraPreset()` | `scene/camera/camera-state:113` | — |
| `getCameraScene()` | `scene/camera/camera-state:293` | — |
| `getCameraVmdName()` | `scene/camera/camera-state:226` | — |
| `getCameraVmdPath()` | `scene/camera/camera-state:230` | — |
| `getConcertParams()` | `scene/camera/camera-state:131` | — |
| `getConcertPaused()` | `scene/camera/camera-state:208` | — |
| `getCurrentCamera()` | `scene/camera/camera-state:188` | — |
| `getFocusCenterY()` | `scene/camera/camera-state:198` | — |
| `getFov()` | `scene/camera/camera-state:178` | — |
| `getFreeflyParams()` | `scene/camera/camera-state:127` | — |
| `getOrbitParams()` | `scene/camera/camera-state:123` | — |
| `getPreviousMode()` | `scene/camera/camera-state:309` | — |
| `getScriptedSubMode()` | `scene/camera/camera-state:156` | — |
| `getSurroundParams()` | `scene/camera/camera-state:135` | — |
| `getSurroundPaused()` | `scene/camera/camera-state:216` | — |
| `getViewMatrixHandle()` | `scene/camera/camera-state:317` | — |
| `hasCameraVmd()` | `scene/camera/camera-state:234` | — |
| `isAutoCameraEnabled()` | `scene/camera/camera-state:250` | — |
| `isTouchDevice()` | `scene/camera/camera-state:276` | — |
| `setAutoCameraBeatCount()` | `scene/camera/camera-state:262` | — |
| `setAutoCameraEnabledFlag()` | `scene/camera/camera-state:254` | — |
| `setAutoCameraPresetIdx()` | `scene/camera/camera-state:270` | — |
| `setCameraBehavior()` | `scene/camera/camera-state:168` | — |
| `setCameraCanvas()` | `scene/camera/camera-state:305` | — |
| `setCameraControl()` | `scene/camera/camera-state:164` | — |
| `setCameraMode()` | `scene/camera/camera-state:160` | — |
| `setCameraPreset()` | `scene/camera/camera-state:117` | — |
| `setCameraScene()` | `scene/camera/camera-state:297` | — |
| `setCameraVmdState()` | `scene/camera/camera-state:238` | — |
| `setConcertPaused()` | `scene/camera/camera-state:212` | — |
| `setCurrentCamera()` | `scene/camera/camera-state:192` | — |
| `setFocusCenterY()` | `scene/camera/camera-state:202` | — |
| `setFov()` | `scene/camera/camera-state:182` | — |
| `setPreviousMode()` | `scene/camera/camera-state:313` | — |
...
| `setEnvState()` | `scene/env/_bridge/env-bridge:315` | 环境状态唯一写入入口（ADR-173 中间件链），可选跳过自动保存。 |
| `setPresetAnimActive()` | `scene/env/_bridge/env-bridge:42` | 标记预设动画是否运行中（供 _applyEnvStateFacade 跳过方向光同步） |
...
| `getGravityStrength()` | `scene/env/env-gravity:28` | — |
...
| `exportCategorizedEnvPreset()` | `scene/env/env-lighting:315` | 序列化分类预设为 JSON 字符串。 |
| `importCategorizedEnvPreset()` | `scene/env/env-lighting:332` | 从 JSON 字符串反序列化分类预设，失败返回 null。 |
...
| `restoreRetargetAnimation()` | `scene/motion/animation-retargeter:234` | 从已加载的模型恢复 retarget 动画（场景反序列化用）。 |
...
| `OverrideType()` | `scene/motion/bone-override:474` | 骨骼覆盖类型（零分配，适合每帧查询） |
...
| `FrameHookManager()` | `scene/motion/motion-modules/module-base:192` | 帧钩子管理器的返回类型（供 createEnsureActive 复用） |
...
| `ModuleShellConfig()` | `scene/motion/motion-modules/module-base:234` | [doc:adr-146 P3 主题12] 模块实例外壳 — 消除 6 个工厂末尾重复的 `id/meta/priority/managedBones/buildSchema + |
| `applyModuleSnapshot()` | `scene/motion/motion-modules/module-base:132` | [doc:adr-125] 将快照应用到指定模型的所有模块。 |
| `createEnsureActive()` | `scene/motion/motion-modules/module-base:213` | [doc:adr-146 P3] ensureActive 公共工厂 — 消除 body-posture/foot/hand 复制粘贴的 「先 bake 重烤、再幂等注册帧钩子」模 |
| `createFrameHookManager()` | `scene/motion/motion-modules/module-base:168` | [doc:adr-116 P3] 帧钩子管理器 — 消除 sway/riding 的 _xxxFrameHooks Map 重复模式。 |
| `createModuleBase()` | `scene/motion/motion-modules/module-base:52` | 创建模块通用方法，减少 7 个模块间 ~105 行重复 boilerplate。 |
| `createModuleShell()` | `scene/motion/motion-modules/module-base:244` | — |
| `prepareBake()` | `scene/motion/motion-modules/module-base:264` | [doc:adr-146 P3 主题13] bake 头部守卫 — 消除 6 个 bake 重复的 `getModuleState + enabled 守卫 + claimBone |
...
| `canRedo()` | `scene/motion/motion-modules/motion-history:166` | 是否有可重做的记录 |
| `canUndo()` | `scene/motion/motion-modules/motion-history:161` | 是否有可撤销的记录 |
| `clearHistory()` | `scene/motion/motion-modules/motion-history:208` | 清除指定模型的历史（删除模型时调用） |
| `getHistoryCursor()` | `scene/motion/motion-modules/motion-history:177` | 获取当前游标位置（UI 高亮用） |
| `getHistoryEntries()` | `scene/motion/motion-modules/motion-history:172` | 获取历史条目列表（UI 显示用） |
| `jumpToHistory()` | `scene/motion/motion-modules/motion-history:186` | [doc:adr-125 P3] 跳转到指定历史位置。 |
| `pushHistory()` | `scene/motion/motion-modules/motion-history:92` | 记录一次参数变更到历史栈。 |
| `redo()` | `scene/motion/motion-modules/motion-history:149` | 重做一步（恢复到下一条快照），返回是否成功 |
| `undo()` | `scene/motion/motion-modules/motion-history:132` | 撤销一步（恢复到上一条快照），返回是否成功 |
...
| `BoneConflict()` | `scene/motion/motion-modules/registry:188` | — |
| `applyMotionModulesToModel()` | `scene/motion/motion-modules/registry:301` | [doc:adr-129] 将场景级模块配置应用到指定模型 用于动作广播时应用配置到所有 inherit 模型 |
| `claimBones()` | `scene/motion/motion-modules/registry:174` | 为模块声明对一组骨骼的所有权（bake 前调用）。 |
| `clearAllModulesForModel()` | `scene/motion/motion-modules/registry:292` | 清除指定模型的所有模块覆盖（删除模型时调用） |
| `createModule()` | `scene/motion/motion-modules/registry:55` | 为指定模型创建模块实例 |
| `getAllConflicts()` | `scene/motion/motion-modules/registry:213` | 获取某模型全部模块的冲突明细（按 loser 模块分组） |
| `getBuiltinModuleDefs()` | `scene/motion/motion-modules/registry:337` | 内置模块定义聚合（供 initMotionModules 批量注册，消除 6 个 registerXxx 分散调用）。 |
| `getConflictCount()` | `scene/motion/motion-modules/registry:235` | 获取某模型冲突总数（骨骼数） |
| `getModuleConflicts()` | `scene/motion/motion-modules/registry:200` | 获取某模块被其他模块抢占的骨骼明细（loser 视角：本模块想要但被谁抢） |
| `getModuleDefaultParam()` | `scene/motion/motion-modules/registry:130` | [doc:adr-116] 读取模块注册的默认参数值。 |
| `getModuleState()` | `scene/motion/motion-modules/registry:83` | 获取动作的模块配置（不存在则创建默认状态，种入 defaults）。 |
| `getOwnedBones()` | `scene/motion/motion-modules/registry:180` | 获取模块当前 owned 的骨骼（disable 时用于精确清除） |
| `getRegisteredModules()` | `scene/motion/motion-modules/registry:48` | 获取所有已注册模块的元信息（按优先级排序） |
| `initMotionModules()` | `scene/motion/motion-modules/registry:349` | 注册所有内置模块（幂等，重复调用安全） |
| `registerModule()` | `scene/motion/motion-modules/registry:33` | 注册一个动作覆盖模块 |
| `releaseOwnedBones()` | `scene/motion/motion-modules/registry:240` | 释放模块的 ownedBones 记录并级联清引擎槽（由 store.releaseBones 负责清除） |
| `setModuleEnabled()` | `scene/motion/motion-modules/registry:152` | 设置模块启用/禁用状态到场景动作意图 |
| `setModuleParam()` | `scene/motion/motion-modules/registry:136` | 写入模块参数到场景动作意图 |
| `setTargetModel()` | `scene/motion/motion-modules/registry:252` | 切换目标模型：禁用当前模型的所有模块覆盖，启用新模型已保存的模块状态。 |
| `unregisterModule()` | `scene/motion/motion-modules/registry:43` | 注销模块 |
...
| `GizmoAttachOptions()` | `scene/render/transform-gizmo:77` | — |
| `GizmoType()` | `scene/render/transform-gizmo:16` | — |
| `attachGizmo()` | `scene/render/transform-gizmo:97` | 为指定 Node 激活变换 Gizmo。 |
| `computeSnapDistance()` | `scene/render/transform-gizmo:58` | 纯函数：给定轴类型与吸附配置，计算吸附步长（场景单位）。 |
| `detachGizmo()` | `scene/render/transform-gizmo:179` | 移除当前 Gizmo。 |
| `getActiveGizmoTypes()` | `scene/render/transform-gizmo:215` | 获取当前激活的 Gizmo 轴类型组合（用于判断拖拽中是否在改缩放）。 |
| `getGizmoNode()` | `scene/render/transform-gizmo:210` | 获取当前 Gizmo 绑定的实时 Node（拖拽中其 transform 已被 Babylon 实时改写，供数值滑杆读取）。 |
| `getGizmoSnapConfig()` | `scene/render/transform-gizmo:251` | 读取当前网格吸附配置（enabled 默认 false，step 默认 1.0）。 |
| `getGizmoTargetId()` | `scene/render/transform-gizmo:205` | 获取当前 Gizmo 绑定的实体 ID。 |
| `initTransformGizmo()` | `scene/render/transform-gizmo:41` | — |
| `isGizmoActive()` | `scene/render/transform-gizmo:195` | 当前是否有 Gizmo 激活。 |
| `isGizmoDragging()` | `scene/render/transform-gizmo:200` | 当前是否正在拖拽 Gizmo（drag start → drag end 之间为 true）。 |
| `onGizmoDragObservable()` | `scene/render/transform-gizmo:37` | 拖拽进行中（连续）可观察量：任一 Gizmo 轴被拖动时每帧触发， 供数值滑杆实时同步显示（ADR-126 Phase 2 双模态）。 |
| `setGizmoSnapDistance()` | `scene/render/transform-gizmo:234` | 设置网格吸附配置。 |
...
| `buildEnvLevel()` | `menus/env-menu:163` | — |
| `buildParticleLevel()` | `menus/env-menu:253` | — |
| `buildParticleSchema()` | `menus/env-menu:171` | — |
| `clearEnvTextureBindingTarget()` | `menus/env-menu:39` | — |
| `disposeEnvMenu()` | `menus/env-menu:73` | 释放 env-menu 模块资源（取消注册 hooks + HMR/清理时调用） |
| `getEnvMenu()` | `menus/env-menu:60` | — |
| `getEnvTextureBindingTarget()` | `menus/env-menu:39` | — |
| `refreshEnvRoot()` | `menus/env-menu:60` | — |
| `showEnvMenu()` | `menus/env-menu:60` | — |
...
| `buildTagDetailLevel()` | `menus/library-actions:689` | — |
| `buildTagsOverviewLevel()` | `menus/library-actions:689` | — |
| `findLibraryModelByName()` | `menus/library-actions:689` | 按名称模糊搜索模型（纯查询，不触发加载）。供 ADR-155/197 NL 控制 resolve 使用，避免 resolve 阶段误触发真实加载。 |
| `findLibraryMotionByName()` | `menus/library-actions:689` | 按名称模糊搜索 VMD 动作（纯查询，不触发替换）。 |
| `highlightRow()` | `menus/library-actions:689` | — |
| `importFile()` | `menus/library-actions:644` | — |
| `importFileByPath()` | `menus/library-actions:599` | — |
| `onModelRowClick()` | `menus/library-actions:689` | — |
| `prepareModelRestore()` | `menus/library-actions:108` | — |
| `replaceModel()` | `menus/library-actions:689` | — |
| `replaceMotion()` | `menus/library-actions:689` | — |
| `makeModelMenu()` | `menus/library-browse:369` | — |
| `showModelPopup()` | `menus/library-browse:338` | — |
| `ResourceViewMode()` | `menus/library-core:43` | — |
| `abortThumbnailStreaming()` | `menus/library-core:305` | [adr-136] 取消当前正在进行的缩略图流式加载批次（如弹窗关闭/重开时调用）。 |
| `buildLevel()` | `menus/library-core:812` | — |
| `buildModelFormationLevel()` | `menus/library-core:855` | — |
| `buildModelRootItems()` | `menus/library-core:886` | — |
| `buildResourceItemsForDir()` | `menus/library-core:372` | — |
...
| `importFile()` | `menus/library-core:980` | — |
| `initLibrary()` | `menus/library-core:981` | — |
| `isLeafFlattenDir()` | `menus/library-core:93` | — |
| `isModelDirTarget()` | `menus/library-core:59` | — |
| `loadThumbnailsStreaming()` | `menus/library-core:250` | 流式加载缩略图：并发控制，每加载一张立即更新缓存并通知面板刷新， 替代一次性 GetThumbnailBatch 的"全等"模式，实现缩略图逐张出现。 |
| `modelToResourceItem()` | `menus/library-core:355` | — |
| `modelToRow()` | `menus/library-core:339` | — |
| `prepareModelRestore()` | `menus/library-core:980` | — |
| `refreshLibrary()` | `menus/library-core:981` | — |
| `refreshModelRoot()` | `menus/library-core:961` | — |
| `reloadConfig()` | `menus/library-core:981` | — |
| `rescanAndSync()` | `menus/library-core:981` | — |
| `resolveDisplayBrowseDir()` | `menus/library-core:140` | [修复] 解析模型在资源库中的"显示目录"——即用户点击该模型时实际看到的层级。 |
| `selectOverridePath()` | `menus/library-core:981` | — |
| `selectResourceRoot()` | `menus/library-core:981` | — |
| `setResourceViewMode()` | `menus/library-core:50` | — |
| `showModelPopup()` | `menus/library-core:979` | — |
| `splitSubdirSegments()` | `menus/library-core:73` | — |
| `switchStorageMode()` | `menus/library-core:981` | — |
