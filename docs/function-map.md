...
| `_resetShortcutRegistry()` | `core/shortcut-registry:419` | Reset all internal state — only for use in tests. |
| `exportKeyBindings()` | `core/shortcut-registry:346` | Get current custom bindings (for saving to uiState). |
| `formatKeyBinding()` | `core/shortcut-registry:219` | 格式化按键绑定为可读字符串，如 "Ctrl+1"、"Shift+←" |
| `getAllShortcuts()` | `core/shortcut-registry:203` | Get all registered shortcuts with their CURRENT effective bindings. |
| `getAriaKeyshortcuts()` | `core/shortcut-registry:255` | 将 ShortcutDef 格式化为 aria-keyshortcuts 值，如 "Control+1" |
| `initShortcutDispatcher()` | `core/shortcut-registry:361` | Initialize the dispatcher — call once at app startup. |
| `loadKeyBindings()` | `core/shortcut-registry:338` | Load custom bindings from persisted state (call at app init). |
| `registerShortcut()` | `core/shortcut-registry:157` | Register ONE shortcut. |
| `registerShortcuts()` | `core/shortcut-registry:196` | Register MULTIPLE shortcuts at once. |
| `resetAllKeyBindings()` | `core/shortcut-registry:330` | Reset ALL shortcuts to their default bindings. |
| `resetKeyBinding()` | `core/shortcut-registry:323` | Reset one shortcut to its default binding. |
| `setKeyBinding()` | `core/shortcut-registry:282` | Set custom key binding for a shortcut ID. |
...
| `_matEnabled()` | `scene/scene:141` | — |
| `_matState()` | `scene/scene:141` | — |
| `animateCameraVmd()` | `scene/scene:841` | — |
| `applyEnvState()` | `scene/scene:828` | — |
| `applyFrameControl()` | `scene/scene:255` | 统一应用帧率控制：帧率限制器开关 + 帧率上限。 |
| `applyMatSssState()` | `scene/scene:167` | — |
| `applyMatState()` | `scene/scene:141` | — |
| `applySss()` | `scene/scene:167` | — |
| `applyUnlitFallback()` | `scene/scene:141` | — |
| `autoFrame()` | `scene/scene:841` | — |
| `autoLoop()` | `scene/scene:799` | — |
| `canUndo()` | `scene/scene:820` | — |
| `captureThumbnail()` | `scene/scene:840` | — |
| `clearCameraVmd()` | `scene/scene:841` | — |
| `disposeModelSssState()` | `scene/scene:167` | — |
| `disposeScene()` | `scene/scene:272` | 级联释放 Scene → Engine 及其所有子资源。 |
| `dom()` | `scene/scene:799` | — |
| `engine()` | `scene/scene:228` | — |
| `envState()` | `scene/scene:799` | — |
| `focusedMmdModel()` | `scene/scene:342` | — |
| `focusedModel()` | `scene/scene:342` | — |
| `focusedModelId()` | `scene/scene:799` | — |
| `formatTime()` | `scene/scene:799` | — |
| `getCameraMode()` | `scene/scene:841` | — |
| `getCameraState()` | `scene/scene:841` | — |
| `getCameraVmdName()` | `scene/scene:841` | — |
| `getCameraVmdPath()` | `scene/scene:841` | — |
| `getMatCatGroups()` | `scene/scene:141` | — |
| `getMatCatParams()` | `scene/scene:141` | — |
| `getMatDetailList()` | `scene/scene:141` | — |
| `getMatParams()` | `scene/scene:141` | — |
| `getMatSssParams()` | `scene/scene:167` | — |
| `getMatSssState()` | `scene/scene:167` | — |
| `getMatState()` | `scene/scene:141` | — |
| `getMaterialCategory()` | `scene/scene:141` | — |
| `getScene()` | `scene/scene:785` | — |
| `hasCameraVmd()` | `scene/scene:841` | — |
| `initCameraSystem()` | `scene/scene:841` | — |
| `initLoader()` | `scene/scene:840` | — |
| `initPlaybackObservables()` | `scene/scene:798` | — |
| `initScene()` | `scene/scene:371` | 场景初始化入口。首次调用时创建 Scene/Engine/运行时； HMR 重入时先调用 _reinitSceneForHMR() 清理旧资源再重建。 |
| `isHeadless()` | `scene/scene:208` | — |
| `isMatCategoryAllEnabled()` | `scene/scene:141` | — |
| `isMatEnabled()` | `scene/scene:141` | — |
| `isPbrMaterial()` | `scene/scene:166` | — |
| `isPlaying()` | `scene/scene:799` | — |
| `loadCameraVmd()` | `scene/scene:841` | — |
| `loadCameraVmdFromPath()` | `scene/scene:792` | — |
| `loadPMXFile()` | `scene/scene:840` | — |
| `loadVMDFromPath()` | `scene/scene:792` | — |
| `loadVMDMotion()` | `scene/scene:792` | — |
| `loadVPDPose()` | `scene/scene:792` | — |
| `mmdRuntime()` | `scene/scene:799` | — |
| `modelManager()` | `scene/scene:315` | — |
| `modelRegistry()` | `scene/scene:799` | — |
| `normPath()` | `scene/scene:819` | — |
| `offerSceneUndo()` | `scene/scene:820` | — |
| `offerSceneUndoAndRefresh()` | `scene/scene:820` | — |
| `popUndoSnapshot()` | `scene/scene:820` | — |
| `pushUndoSnapshot()` | `scene/scene:820` | — |
| `resetMatCatParams()` | `scene/scene:141` | — |
| `resetPerMaterialParams()` | `scene/scene:141` | — |
| `resetSingleMatParams()` | `scene/scene:141` | — |
| `resolveFileUrl()` | `scene/scene:819` | — |
| `restoreUndoSnapshot()` | `scene/scene:820` | — |
| `scene()` | `scene/scene:239` | — |
| `seekDragging()` | `scene/scene:799` | — |
| `seekFromEvent()` | `scene/scene:798` | — |
| `setAutoLoop()` | `scene/scene:799` | — |
| `setCameraState()` | `scene/scene:841` | — |
| `setFocusedModelId()` | `scene/scene:799` | — |
| `setIsPlaying()` | `scene/scene:799` | — |
| `setMatCatParams()` | `scene/scene:141` | — |
| `setMatCategoryEnabled()` | `scene/scene:141` | — |
| `setMatEnabled()` | `scene/scene:141` | — |
| `setMatParams()` | `scene/scene:141` | — |
| `setMatSssParams()` | `scene/scene:167` | — |
| `setMmdRuntime()` | `scene/scene:799` | — |
| `setModelRegistry()` | `scene/scene:799` | — |
| `setSeekDragging()` | `scene/scene:799` | — |
| `setStatus()` | `scene/scene:799` | — |
| `setTriggerAutoSave()` | `scene/scene:817` | — |
| `switchCameraMode()` | `scene/scene:841` | — |
| `triggerAutoSave()` | `scene/scene:817` | — |
| `updatePlaybackUI()` | `scene/scene:798` | — |
