...
> 核心命题：从「工具」到「平台」—— 让 MikuMikuAR 成为 MMD/PMX 生态的事实标准桌面入口。

### 🌱 平台化路径（3 年路线）

```
2026 Q3-Q4   Android Beta → 公测 → v1.0 桌面版发布
  ↓
2027 Q1-Q2   iOS/iPadOS 适配 + 插件系统 v1（脚本动作/自定义粒子）
  ↓
2027 Q3-Q4   生态集成（DanceXR 场景导入 + Blender 官方适配）+ 开放 API
  ↓
2028+        云端同步 + 社区协作 + 跨平台生态（模型库 marketplace）
```

### 🎯 三年目标（按维度）

**平台扩展**
- 📋 iOS/iPadOS 适配（2027 Q1-Q2）：SwiftUI 触控适配，支持 iPhone/iPad 多尺寸
- 📋 macOS/Linux 原生打包（2027）：通过 Wails 实现真正的原生体验（非 Electron）

**生态集成**
- 📋 **DanceXR 深度集成**：`.pose` / `.anim` / `.stage` 场景包一键导入，格式规范文档化
- 📋 **Blender 官方适配**：与 mmd_tools 团队合作，提供「MMD 场景 → MikuMikuAR 导出」一键插件
- 📋 **模型 marketplace**：开发者/画师可上架付费/免费模型包

**插件系统**
- 📋 **插件沙箱 v1**（2027 Q1-Q2）：基于 Wails IPC 的安全隔离，支持 Python/Lua 脚本热插拔
  - 可扩展场景：自定义 VMD 生成器（AI 动作迁移）、自定义后处理着色器、自定义粒子特效
- 📋 **插件市场**：GitHub Actions CI/CD 自动构建插件包，审核机制待定

**开放 API**
- 📋 **REST API v1**（2027 Q3-Q4）：标准化的 `.mmascene` 序列化规范 v2，第三方工具可读写场景
- 📋 **WebAssembly 插件**：插件编译为 WASM，跨语言扩展（Rust/C++ 性能关键模块）

**社区与内容**
- 📋 **用户反馈体系**：GitHub Issue 模板化 + 社区 Discord/QQ 群
- 📋 **国际化（i18n）**：英语/日语/中文三语 UI 支持

### 🔭 远期愿景（5 年+

- **AI 驱动创作**： Diffusion 模型辅助生成场景描述词 → 自动设置环境/灯光
- **实时协作**：多用户同时编辑同一场景（CRDT 冲突解决）
- **AR/VR 入口**：ARKit/ARCore 移动端 AR 预览；Meta Quest 虚拟演唱会模式
