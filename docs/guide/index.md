...
> 按功能讲解入口路径与操作步骤，共 **28** 篇。新功能持续建档，重跑 `npm run gen:docsindex` 自动入列。

| 指南页 | 说明 |
|--------|------|
| [AI 助手](./ai-assistant.md) | 内置 AI 诊断助手：对话答疑、自然语言控场、配置与会话管理 |
| [相机操作](./camera-control.md) | 轨道/自由飞行相机切换，旋转/缩放/平移，以及相机参数微调 |
| [云层设置](./cloud.md) | 天空动态云层：密度、移动速度与透明度 |
| [拖拽模式](./drag-mode.md) | 用鼠标直接拖拽移动模型/道具，吸附对齐与位置微调 |
...
| [实验功能](./experimental.md) | 尚未转正的新特性试验场，默认带警告提示 |
| [外部动作导入](./external-import.md) | 把 Mixamo / VRM 等非 MMD 动作重定向到当前模型上播放 |
| [雾设置](./fog.md) | 环境雾：颜色、密度与远近范围，营造远景层次 |
| [队形编排](./formation.md) | 多模型队形：网格/圆形等批量排列，一键舞台站位 |
| [视线追踪](./gaze-tracking.md) | 让角色的头部/眼球跟随相机，呼吸眨眼等程序化生命力微调 |
| [地面设置](./ground.md) | 地面网格/样式/高度调整，阴影开关与地面跟随 |
| [模型导入](./import-model.md) | 从模型库加载 PMX/PMD 模型到舞台，附导入路径与常见问题 |
| [镜面反射](./mirror.md) | 平面反射/SSR/探针：舞台地面镜面效果 |
...
| [姿势工作室](./pose-studio.md) | 构图辅助线、T-pose/A-pose 姿势预设、相机角度与批量截图 |
