...
| ADR | 主题 | 状态 |
|-----|------|------|
| ADR-262 | Vitest isolate:false + vi.mock 污染机制、金丝雀诊断与护栏 | ✅ 已采纳（2026-08-16） |
| ADR-261 | 测试环境分流实证修正 — 隐式 DOM 依赖不是表面嗅探能捕捉的 | ✅ 已采纳（2026-08-16） |
| ADR-260 | pre-push 检查并行化 | ✅ 已采纳（2026-08-18） |
| ADR-259 | pre-push 检查缓存机制 — 基于文件哈希的缓存，避免重复计算 | 🔄 部分采纳（2026-08-17） |
| ADR-258 | 风险与收益量化分析 | 📊 参考文档（非决策）（2026-08-17） |
| ADR-258 | 修订版：pre-push 门禁分层与分流（数据驱动） | ✅ 已采纳（2026-08-17 修订）（2026-08-16（初版）→ 2026-08-17（数据驱动修订）） |
| ADR-258 | pre-push 门禁分层与改动面分流 | ✅ 已采纳（2026-08-17）（2026-08-16（初版）/ 2026-08-17（数据驱动修订）） |
| ADR-258 | 数据报告：pre-push 门禁量化证据 | 📊 参考文档（非决策） |
...
| ADR-252 | 菜单资源释放链路缺失 —— disposeMenuWrapper/clearAllMenuWrappers 零调用与 _liveMenus 常驻 | ✅ 已采纳（2026-08-11 实施；closeAllOverlays 内联 clearAllMenuWrappers + HMR dispose 接线，消除零调用死代码）（2026-08-06） |
...
| ADR-241 | .mjs 脚本文档统一约定（跨仓库硬约束） | ✅ 已实施（2026-08-05 完成；两端 README 同款节 + 四口径检查器 `check-script-hygiene.mjs` 落地并自执行） |
...
| ADR-233 | 程序化动作 per-mode 独立参数 —— 拆分单一状态为待机/自动舞蹈各自参数集 | ✅ 已完成（2026-08-02 落地，全量单测绿） |
| ADR-232 | ADR 元治理 —— 取代关系判别与脚本口径统一 | 已落地（2026-08-02，多轮迭代后收口） |
| ADR-231 | 地面视觉后续方向（自发光地屏 + 程序化地面图案） | 📝 规划（2026-08-02） |
| ADR-230 | 文档自动化工具链决策 —— 从手写索引到机器守护的完整闭环 | 已落地（2026-08-02，十轮抽样迭代后收口） |
| ADR-229 | E2E 自动化推进 —— 从 schema 到测试零映射 | 部分实施 — Phase 1（导航自动化 + CI 快照门禁）与 Phase 2 主体（action 交互 + `__state` + DOM 契约统一）已落地（2026-08-02，schema-driven E2E 30/30 全绿）；Phase 3（视觉回归 + WebGL 推广）待做 |
...
| ADR-211 | 水面功能开关体系 — 噪声原语消重与效果分组开关化 | ✅ Part 1/2/3 完成（大波·焦散·水下效果开关全部落地；平面反射由 ADR-151 `reflectionMode='none'` 接管，豁免独立开关，详见 Part 3）（2026-07-30） |
| ADR-210 | 环境光照字段名名实相符重命名（envIntensity/envBrightness） | ✅ 已完成（2026-07-30） |
...
| ADR-198 | 场景序列化异常的保存韧性 | 已实施（2026-07-31）（2026-07-28） |
...
| ADR-188 | PBRMaterialBuilder 材质系统迁移 — PBR 渲染升级 | Phase 1 完成（2026-08-01） |
...
| ADR-162 | 感知层 per-model 实例化 — Phase 1（pinned 模型支持） | 已完成（2026-07-21；per-model 真实隔离由 ADR-166 收口；独立审核 frontend 1821 测试 0 失败）⚠️ **§6 验收标准已过时** — pin 功能已随 ADR-164/166 整合入 tier 系统（见 §六 勘误） |
...
| ADR-156 | 大模型交流 — 创意路线（角色台词生成） | 🟡 Step 1 + Step 2a 已实施（台词生成 + 情绪卡片 + TTS 朗读）；Step 2b（口型闭环）待评估（2026-07-28）（2026-07-20（初版），2026-07-28（Step 1+2a 落地回填）） |
| ADR-155 | 自然语言控场景 — 叠加于 AiService 管线之上 | 🟢 已完成（2026-07-20（初版），2026-07-28（重写），2026-07-28（终版对齐实现）） |
| ADR-154 | 引入大模型交流能力 — 推荐路线（聊天面板打底） | 🗑️ 已被 ADR-196 取代（Superseded）—— 本路线的技术目标（Go LLM 客户端 + 流式管线 + 聊天面板）已由 ADR-196 提前达成，架构合并（聊天 = 诊断面板「闲聊」tab 子集），无独立落点存续（2026-07-20（初版），2026-07-28（归档为被取代）） |
...
| ADR-28 | 风场系统统一 — 从碎片化到集中治理 | 已实现 — wind-utils.ts 统一风向量 + 云/粒子/水面/布料四子系统联动（风量物理生效见 ADR-194）（2026-07-05） |
