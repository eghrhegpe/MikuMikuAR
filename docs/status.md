...
Wails (Go) + babylon-mmd 的桌面/移动 PMX 查看器。核心管线、模型库管理、多模型场景、粒子系统、程序化动作、换装系统、环境系统、Android 适配、AR 相机（Phase 1 桌面 MVP）均已就绪。卡通化渲染（ADR-076）已实施。菜单声明式 Schema 架构（ADR-093）57 个面板迁移完成。v1.3.5 已发布（ADR-095/096/097/098/100/102/103 批量重构落地）。SettingsStore 已移除，所有设置统一归入 uiState 持久化链路。
...
| 广场浏览器体验增强（ADR-087） | 🔄 实施中 |
| MPR/COOP/COEP（ADR-099） | 🔄 部分实现 |
...
| ADR-101 | 通用逻辑模式收敛第二波（错误处理/生命周期守卫/纯函数，~350+ 处重复模式） | 📋 规划中（2026-07-13） |
| ADR-102 | main.ts 拆分（init/events/render-loop/dev-hooks，P0-P5 全阶段落地） | ✅ 已实施（2026-07-13） |
| ADR-103 | SettingsStore 移除 + 设置统一归入 uiState 持久化链路 | ✅ 已实施（2026-07-13） |
...
| ADR-087 | 广场浏览器体验增强 | 🔄 实施中 | P0 已完成（下载拦截代理桥接 + 导航控制 + 遥控面板）；P1/P2 待推进 |
| ADR-088 | 音效系统—脚步声与 SFX 总线 | 🔄 部分实现 | Phase A 已完成（audio-bus + footstep.ts）；Phase B/C 待开发 |
| ADR-093 | 菜单声明式 Schema | 🔄 实施中 | P0+P1+P2 实质完成；P3 收尾中 |
| ADR-099 | MPR/COOP/COEP POC | 🔄 部分实现 | Go 半侧 POC 已落地并提交；前端 `MmdWasmInstanceTypeMPR` 切换因 Vite 构建阻断，暂缓 |
| ADR-100 | 相机行为双轴控制 | 🔄 实施中 | P1（契约+shim）+ P2（运行时接线）+ P3-P5（序列化+双写+两级 UI）已提交；P6 收尾中 |
| ADR-101 | 通用逻辑模式收敛第二波 | 📋 规划中 | ~350+ 处重复模式（错误处理/生命周期守卫/纯函数），分 P1/P2/P3 三阶段 |
