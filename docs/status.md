...
Wails (Go) + babylon-mmd 的桌面/移动 PMX 查看器。核心管线、模型库管理、多模型场景、粒子系统、程序化动作、换装系统、环境系统、Android 适配、AR 相机（Phase 1 桌面 MVP）均已就绪。卡通化渲染（ADR-076）已实施。菜单声明式 Schema 架构（ADR-093）57 个面板迁移完成。v1.3.5 已发布（ADR-095/096/097/098/100/101/102/103 批量重构落地）。SettingsStore 已移除，所有设置统一归入 uiState 持久化链路。MPR/COOP/COEP 真机验证通过（ADR-099）。广场浏览器体验增强全量完成（ADR-087）。
