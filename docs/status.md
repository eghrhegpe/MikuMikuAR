...
| # | 产出 | 指标 | 验收 |
|---|------|------|------|
| P0 | `.apk` 安装包（Google Play 发布格式） | Pixel 6/7/8 + Galaxy S21 启动成功率 ≥ 90%；加载 500 面 PMX ≤ 3s；播放 30s VMD 掉帧率 < 5% | 手动真机测试 × 3 台 + GitHub Actions 自动化回归 |
| P1 | `app.go` 错误日志统一 + `MenuStack` 生命周期整合 | `tsc --noEmit` 错误 ≤ 5；无新增 `// TODO: P0/P1` | CI lint check + PR review |
| P1 | 惯性滑动调校完成 + 系统夜间模式跟随正常 | 触控手感评分 ≥ 4/5（5 人内部测试）；`prefers-color-scheme` 检测生效 | 用户测试 + CSS media query 验证 |
| P2 | 渲染管线调研报告 `docs/research/rendering-upgrade.md` | 覆盖 ≥ 3 种方案（PBR/Custom/URP），含性能对比数据 + 推荐结论 | 技术评审会议 + reviewer 签字 |

**Q4（2026-10 ~ 2026-12）：Android 公测 + v1.0 桌面版 + 生态集成**

| # | 产出 | 指标 | 验收 |
|---|------|------|------|
| P0 | Android 公测包（APK + 内部测试链接） + v1.0 changelog | 100 名测试用户；NPS ≥ 40；Crash-free sessions ≥ 98% | TestFlight/Google Play internal track 收集反馈 |
| P0 | Win/macOS/Linux 安装包（`.exe` / `.dmg` / `.AppImage`） | CI 三平台 build 均通过；安装后首次启动成功率 ≥ 95% | VM 自动化安装测试 + 3 台物理机验证 |
| P1 | 插件沙箱 v0.1 + 1 个 Python 示例插件（自定义 VMD 生成） | 恶意插件不破坏主程序（隔离测试通过）；示例加载 ≤ 2s | 安全测试报告 + CI sandbox test |
| P1 | DanceXR `.pose` 导入器（至少支持 `.pose` 文件） + Blender 导出脚本 | `.pose` 导入成功率 ≥ 80%；Blender 脚本 CI 测试通过 | 自动化测试 10 个样本文件 + 用户验证 |
| P2 | `docs/research/plugin-api.md`（插件 API 设计文档） | 至少覆盖 3 个扩展点（动作/后处理/粒子）；有 sequence diagram | API review + 至少 1 个外部开发者反馈 |

**Q1-Q2（2027）：平台扩展 + 开放 API**

| # | 产出 | 指标 | 验收 |
|---|------|------|------|
| P2 | iOS TestFlight 公测包 + iPad 多尺寸适配 | iPhone 14 启动成功率 ≥ 90%；iPad 分辨率无 UI 溢出 | 真机测试 + App Store TestFlight |
| P3 | `.mmascene` 序列化规范 v2 文档 + `docs/api/v1/openapi.yaml` | 至少 1 个第三方工具集成（VMD 可视化器 / 模型转换工具） | 社区 Issue 确认集成成功 |
