...
Phase 2     渲染调参系统                      ← ✅ 已完成
              ...
              └── 线框/骨骼显示切换 — ❌ 下一轮
              ↓
Phase 3     音乐同步 + 相机VMD               ← 竞品对标，新增 P1-P2
              ├── 音乐同步(MP3/WAV/OGG) + 音频偏移
              ├── 相机 VMD 轨道
              └── 舞蹈套装(VMD+音频捆绑)
              ↓
Phase 4     下载目录监听 + 自动导入 (ADR-008) ← 原 Phase 2
              ↓
Phase 5     体验完善                          ← 新增
              ├── 模型统计信息 / 表情预览
              ├── 批量截图 / 导出到MMD
...
从「骨架搭建」转向「体验打磨」：
1. **标签系统** — 自定义标签替代 DanceXR 八分类硬编码
2. **下载目录监听** — fsnotify 监听 + 一键导入 (ADR-008)
