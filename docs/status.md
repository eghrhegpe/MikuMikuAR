...
| 风格化水体波光粼粼调研（ADR-115） | 📋 规划 |
| 动作覆盖系统 Bone Override → Motion Override（ADR-116） | ✅ |
| Go 端错误 i18n 化（ADR-117） | 📋 规划 |
| 刷新率感知自动降级（ADR-118） | ✅ Phase 1 |
| 缩略图缓存键单一源治理（ADR-119） | ✅ 治理闭环 |
| 环境预设分类化（ADR-120） | 🔄 实施中 |
| 全局动作意图 Scene-level Motion Intent（ADR-121） | 📋 规划 |
...
| ADR-115 | 风格化水体波光粼粼调研 | 📋 规划 |
| ADR-116 | 动作覆盖系统 Bone Override → Motion Override 重构 | ✅ 已完成（P0-P3 全部实施） |
| ADR-117 | Go 端错误 i18n 化 | 📋 规划 |
| ADR-118 | 刷新率感知自动降级 | ✅ Phase 1 完成 |
| ADR-119 | 缩略图缓存键单一源治理 | ✅ 治理闭环 |
| ADR-120 | 环境预设分类化（天空/地面/水面/大气） | 🔄 实施中 |
| ADR-121 | 全局动作意图 Scene-level Motion Intent | 📋 规划 |
