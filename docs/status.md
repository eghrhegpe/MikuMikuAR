...
| ADR-153 | 无障碍支持总体方案 | ✅ 全部完成（三阶段） | P1 焦点环 + aria-live + focus trap + canvas ARIA；P2 prefers-* 媒体查询 + Android 返回键 + 3D 键盘轨道控制 + 模型 alt；P3 aria-label 补全 + aria-keyshortcuts + axe E2E + ui-keyboard-nav + Go 系统主题桥接。⚠️ Phase 3.4 例外：menu.ts 方向键因语义不兼容保留自有实现 |
...
| ADR-162 | 感知层 per-model Phase 1 | ✅ 已完成（2026-07-21，由 ADR-166 收口） | per-model 真实隔离：Map<modelId, Context> + pin API + 序列化迁移；独立审核 frontend 1821 测试 0 失败 |
| ADR-163 | 感知层冲突可视化 | ✅ 已完成（2026-07-21，由 ADR-166 收口） | claimBones + banner + 滑块冲突标记 + P2-1 重 claim / P2-2 unpin / P2-3 banner 泛化 |
...
| ADR-166 | 感知层 per-model 真实隔离返工（收口 ADR-162/163） | ✅ 已完成（2026-07-21，P4-4 对象池 2026-07-22） | 真实 per-context 隔离 + 对象池；tsc 0 错误 / frontend 1829 测试全绿 |
| ADR-167 | 场景级动作库（多主动作平等共存） | 📋 规划中 | 路线规划，无代码落地 |
| ADR-168 | 动态追光：舞台灯跟随角色/骨骼 | 🔄 部分实现 | 基础追光接入，部分场景待完善 |
| ADR-169 | 动作装载语义统一（replaceDefaultMotion 原位替换） | ✅ 已实施 | replaceDefaultMotion 函数 + vmd-loader 迁移 + 撤销快照 + 6 单测全绿 |
| ADR-170 | 动作库选中范式（默认暴露为逐行「选中」） | ✅ 已实施（2026-07-22, 9ac064d9） | 对齐模型焦点范式 |
| ADR-171 | 场景级拖拽模式（快捷开关 + 收纳文件夹） | ✅ 已完成 | 拖拽模式快捷开关 + 收纳 |
| ADR-172 | 湿身效果：雨天角色材质湿润感 | ✅ 已完成 | env-wetness.ts 材质湿润感 |
| ADR-173 | env-bridge setEnvState 中间件化重构 | ✅ 已完成 | env-bridge.ts 中间件化 |
| ADR-174 | 质量维度注册表 — 统一 qualityProfile 扩展点 | ✅ 已完成 | quality-profile.ts 注册表 |
| ADR-175 | 光照强度多入口设计意图裁决 | ✅ 已裁决（无代码改动，固化契约） | 保留多入口，不收敛为单入口 |
| ADR-176 | 前端 Backend 适配器双实现（Web/Desktop 通杀） | ✅ 已实施（2026-07-23 Phase 1/2/3 全部落地） | BackendService 接口 + go-adapter/browser-adapter 双实现；106 方法 browser-adapter；wails-bindings.ts 绞杀者代理；web-loader 准完整网页入口 |
---
