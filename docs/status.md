...
| ADR | 主题 | 状态 |
|-----|------|------|
| ADR-197 | 统一动作注册表 — 菜单可维护性归一化 | 🟢 实施中（Phase 0 已完成）（2026-07-28） |
| ADR-196 | 内置 AI 诊断助手（LLM Diagnostic Assistant） | ✅ 已完成（Phase 0 基础设施 + Phase 1 集成打通与体验打磨）（2026-07-28） |
...
| ADR-155 | 自然语言控场景 — 叠加于 AiService 管线之上 | 🔄 实施中（已进入 Phase 1 实施阶段，基于 ADR-196 Phase 0 传输层）（2026-07-20（初版），2026-07-28（重写，对齐 ADR-196）） |
