...
| IK 感知骨骼覆盖（ADR-122） | 📋 规划中 | ADR-122 状态行权威：`applyBoneOverrideIK` 未实现 |
| 计算覆盖语义规范（ADR-123） | 📋 规划中 | ADR-123 状态行权威：`absolute` 模式未实现 |
...
| 缩略图流式加载 AbortSignal + 弹窗重开钩子（ADR-137） | ✅ |
| 材质覆盖基线冲突解决（ADR-149） | ✅ |
...
| ADR-093 | 菜单声明式 Schema（`MenuNode` + `renderMenu()` 单渲染器，57 面板迁移完成：env/motion/scene/model/settings 全域覆盖） | ✅ P0+P1+P2 已实施（P3 收尾待推进） |
...
| ADR-122 | IK 感知骨骼覆盖 | 📋 规划中 | ADR-122 状态行权威：代码未实现 |
| ADR-123 | 计算覆盖语义规范 | 📋 规划中 | ADR-123 状态行权威：代码未实现 |
| ADR-124 | 文件系统 fileaccess 迁移 | ✅ 已完成 | Phase 1-3 全量落地（readFileBytes Uint8Array 适配 + 纹理递归直传 + 死代码移除） |
| ADR-125 | 动作覆盖撤销/重做 | ✅ 已实施 | P1+P2+P3 完成（pushUndoSnapshot/restoreUndoSnapshot/offerSceneUndo + info toast） |
| ADR-126 | 变换适配器注册表去重 | ✅ 已完成 | Phase 1-3 全量落地 |
| ADR-127 | 场景级破坏性操作撤销 | ✅ 已实现 | Memento 快照 + 撤销 Toast |
| ADR-128 | 镜面道具化重命名 | ✅ 已完成 | debugMirror → mirror 全量迁移，i18n 5 语种无残留 |
...
| ADR-132 | 环境亮度统一标量 | 📋 规划 |
| ADR-135 | 缩略图流式加载方案（library-session-store 升级） | ✅ 已完成 | P0-P4 全量落地 + 真机验证 + 旧代码移除 |
...
| ADR-137 | 缩略图流式加载 AbortSignal + 弹窗重开钩子 | ✅ 已完成 | loadThumbnailsStreaming / loadThumbnailsForLevel 接收 + 透传 AbortSignal，弹窗重开时 abort 旧请求 |
| ADR-139 | 动作覆盖菜单重构方案 | ✅ 已完成 | per-motion 声明式预设系统 + preset-list-viewer + MotionIntent 解耦 |
| ADR-140 | 动作叠加与撤销 | ✅ 已完成 | per-motion undo snapshot + offerSceneUndo |
| ADR-141 | 动作覆盖撤销/重做 | ✅ 已完成 | Memento 快照 + 撤销 Toast |
| ADR-142 | 模型替换契约 | ✅ 已完成 | modelReplaceTargetId → activateItem 统一派发 |
| ADR-149 | 材质覆盖基线冲突解决 | ✅ 已完成 | overlay baseline 独立于 applyOutfit，覆盖后 outfit 可恢复 |
| ADR-150 | 模型替换与浏览器选中结果统一契约 | ✅ 已完成 | BrowseOutcome.activateItem 统一契约 |
| ADR-151 | 反射系统统一架构 | ✅ 已完成 | render-passes 模式 + 实时/烘焙双模 |
| ADR-152 | 舞台灯光与体积光散射 | ✅ 已完成 | 真光锥 + 体积散射统一模块 |
---
...
| ADR-120 | 环境预设分类化（天空/地面/水面/大气） | ✅ Phase 1 完成 | Go API + bindings + TS 分类预设 + UI 重写 + i18n + 单测全绿；待真机验证 |
| ADR-121 | 全局动作意图 Scene-level Motion Intent | ✅ 已完成 | 审计 7 项修订闭环，已具备进 P0 条件 |
| ADR-122 | IK 感知骨骼覆盖 | 📋 规划中 | ADR-122 状态行权威：`applyBoneOverrideIK` 未实现 |
| ADR-123 | 计算覆盖语义规范 | 📋 规划中 | ADR-123 状态行权威：`absolute` 模式未实现 |
...
| ADR-141 | 动作覆盖撤销/重做 | ✅ 已完成 | Memento 快照 + 撤销 Toast |
| ADR-142 | 模型替换契约 | ✅ 已完成 | modelReplaceTargetId → activateItem 统一派发 |
| ADR-144 | 模型层运动预设系统 | 📋 规划中 | ADR-144 状态行权威：`_ensureOverlayLayer` 未实现 |
| ADR-145 | 动作预设方案 | 📋 规划中 | ADR-145 状态行权威：`applyMotionPreset`/`preset-types.ts` 未实现 |
| ADR-146 | 函数功能重复审查 | 📋 规划中 | 审查进行中 |
| ADR-148 | 过载模块拆分 | 📋 规划中 | 审查进行中 |
| ADR-149 | 材质覆盖基线冲突解决 | ✅ 已完成 | overlay baseline 独立于 applyOutfit，覆盖后 outfit 可恢复 |
| ADR-150 | 模型替换与浏览器选中结果统一契约 | ✅ 已完成 | BrowseOutcome.activateItem 统一契约 |
| ADR-151 | 反射系统统一架构 | ✅ 已完成 | render-passes 模式 + 实时/烘焙双模 |
| ADR-152 | 舞台灯光与体积光散射 | ✅ 已完成 | 真光锥 + 体积散射统一模块 |
| ADR-153 | 无障碍支持总体方案 | 🔄 实施中 | Phase 1 已落地，Phase 2/3 规划中 |
