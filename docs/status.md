...
| ADR | 主题 | 状态 |
|-----|------|------|
| ADR-202 | fork 自治改动批次 — 一次回灌批量根治可改 fork 的上游缺口 | 🟡 草案（承重项 P0「fork 产物 CI 可复现分发」未解决前，任何 fork 改动不得进发版） |
| ADR-201 | 路径2 评估 — 给 babylon-mmd fork 增加原生刚体 JS 句柄以施风力 | 🟡 草案（评估完成 — **结论：可行，投入低-中**，已具备全部承重构件） |
...
| ADR-85 | 脚部地面跟随（Feet Adjustment） | Phase A 已完成；Phase B/C 降级搁置（2026-07-19）· 已纳入代码审核 4 项修正（纯文档，无代码变更）· 方案C（WASM 手动 IK）已实施（2026-07-26）（2026-07-11） |
