...
> **状态**: ✅ 核心已实施 — render↔manager 互依赖已拆解（2026-08-03，texture-lru 下沉 scene/shared，互环归零）；剩余 21 个新增环由 [ADR-238](adr-238-循环依赖消解二期-core-scene-根环.md) 承接并已拆至 11 环（2026-08-03 收尾）。**订正**：texture-lru 下沉 scene/shared 引入 `scene/shared → core` 边，是 ADR-238 环 ①（`core→scene→motion-algos→scene/env→scene/render→scene/shared→core`）的成因之一，已在 ADR-238 §6 记录，勿误判为「剩余环不含 render」。门禁转正待剩余 11 环合理保留确认。
> **日期**: 2026-08-03
>
...
- **环归因工具**（check-circular 扩展，2026-08-03）：
  ...
  - `--diff <file>`：与基线对比，标出新增环、已消失环、以及每个新增环路径上**基线中不存在的模块对边**（`[新增边]` 标注）——定位「哪条新 import 引入了环」；`--diff --strict` 按是否有新增环决定退出码
