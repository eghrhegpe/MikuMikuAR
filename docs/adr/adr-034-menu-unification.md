...
| 组件 | 角色 | 逐级特殊化 |
|------|------|-----------|
| `cardContainer()` | renderCustom 菜单的卡片容器 | 通用（仅 renderCustom） |
| `buildPanel()` divider 分组 | 纯 items 菜单自动按 divider 分组创建 `.lcard` | 自动（仅 items） |
| `slideRow()` | 菜单项：图标 + 标签 + 箭头/详情/toggle | ↓ |
...
不在 `cardContainer` 外直接创建 `lcard`，不在面板内手动创建 `slide-item`。items 菜单用 `divider` 分组，`buildPanel()` 自动包裹 `.lcard`。
