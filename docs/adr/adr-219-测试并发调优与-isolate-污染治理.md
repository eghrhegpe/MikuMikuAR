...
### 勘误（2026-08-16，ADR-262 实证修正）

ADR-219 三次证伪将残余失败定性为"模块级单例 mock 穿透"，认为"形状收口无效、结构性不可修"。
2026-08-16 ADR-262 用 `isolate:false` 作为金丝雀扫描器（四轮实验、3 轮 shuffle + 1 轮无
shuffle），对残余技术债务做了更精确的实证分类，**修正 ADR-219 的部分定性**：

| ADR-219 原定性 | ADR-262 实证修正 |
|---------------|------------------|
| "vi.mock 单例穿透结构性不可修" | 残余债务分三类：**稳定失败 6 个**（4/4 轮命中，与顺序无关，是模块级单例状态 `env-context._scene`/engine 实例/observable 订阅泄漏）、**强信号 5 个**（3/4 轮命中，结构性 + 顺序叠加）、**飘移失败 6 个**（1/4 轮命中，纯顺序敏感）——vi.mock 只是入口，**深层污染源是模块级单例状态** |
| "isolate=false 不可用"（结论） | **保留**。vi.mock 污染修掉后从 229 降到 60~110，但结构性债务仍在，isolate=false 不可用 |
| 无 | 新增技术债务清单（9 项，P0~P3），按出现频次排序，明确修复方向（参数化/DI/工厂模式） |

**ADR-219 最终结论不变**：`isolate:false` 在当前代码库不可用。但理由从"vi.mock 形状穿透"
修正为"**模块级单例状态泄漏 + vi.mock 入口污染**两层叠加"，前者是更深层的架构债务，
需要源码级重构而非测试层 mock 策略解决。详见 [ADR-262 §五](adr-262-vitest-isolate-false-vi-mock-pollution-mechanism-and-guardrails.md#五实证isolatefalse-技术债务扫描2026-08-16)。
