...
- [ADR-151](adr-151-reflection-unified-architecture.md) — 反射系统统一架构：经 `getPlanarQualityOverride` 协调本 ADR 水面反射分辨率（`none`→强关、`planar`→至少 `low`），SSR/Probe 单源收口到 `reflectionMode`/`reflectionQuality`
