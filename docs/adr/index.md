...
| ADR | 主题 | 状态 |
|-----|------|------|
| [ADR-244](./adr-244-init-phase-split.md) | init 启动流程阶段化拆分 —— 110 行编排器按职责切分 | 🔄 规划中 —— 已登记方案与实施步骤，待落地 |
| [ADR-243](./adr-243-env-state-defaults-from-schema.md) | EnvState 默认值从 Schema 自动推导 —— 消除 100+ 字段双源手工映射 | 🔄 规划中 —— 已登记方案与实施步骤，待落地 |
