...
> 架构决策日志，共 **261** 篇（ADR-001 起按编号递增）。决策一旦写下即不可变；状态变化以各 ADR 文件首部「状态」行为准。
...
| [规划中](#规划中) | 7 | 已立项，等待实施 |
| [已落地](#已落地) | 235 | 实施完成，代码已合入 |
| [已归档](#已归档) | 13 | 被取代、放弃、过时或搁置，保留供追溯 |
...
| ADR | 主题 | 状态 |
|-----|------|------|
| [ADR-262](./adr-262-vitest-isolate-false-vi-mock-pollution-mechanism-and-guardrails.md) | Vitest isolate:false + vi.mock 污染机制、金丝雀诊断与护栏 | ✅ 已采纳 |
| [ADR-261](./adr-261-测试环境分流实证修正-隐式-dom-依赖不是表面嗅探能捕捉的.md) | 测试环境分流实证修正 — 隐式 DOM 依赖不是表面嗅探能捕捉的 | ✅ 已采纳 |
| [ADR-260](./adr-260-pre-push-parallel.md) | pre-push 检查并行化 | ✅ 已采纳 |
| [ADR-259](./adr-259-pre-push-检查缓存机制.md) | pre-push 检查缓存机制 — 基于文件哈希的缓存，避免重复计算 | 🔄 部分采纳 |
| [ADR-258](./adr-258-pre-push-门禁分层与改动面分流.md) | pre-push 门禁分层与改动面分流 | ✅ 已采纳 |
| [ADR-258](./adr-258-revised.md) | 修订版：pre-push 门禁分层与分流（数据驱动） | ✅ 已采纳 |
| [ADR-257](./adr-257-maxworkers-8.md) | maxWorkers 12→8：共享机器并发场景的资源经济学 — 并发实测：双 vitest 各 12 worker（24 核拉满）69.4s，各 8 worker 64.3s——24 核全占后 CPU 竞争使墙钟反超；单 AI 8 核仅慢 3.7s（9%） | ✅ 已采纳 |
...
| [ADR-241](./adr-241-mjs-script-doc-convention.md) | .mjs 脚本文档统一约定（跨仓库硬约束） | ✅ 已实施 |
...
| [ADR-232](./adr-232-adr-meta-governance.md) | ADR 元治理 —— 取代关系判别与脚本口径统一 | 已落地 |
| [ADR-230](./adr-230-docs-automation-toolchain.md) | 文档自动化工具链决策 —— 从手写索引到机器守护的完整闭环 | 已落地 |
...
| ADR | 主题 | 状态 |
|-----|------|------|
| [ADR-258](./adr-258-data-report.md) | 数据报告：pre-push 门禁量化证据 | 📊 参考文档 |
| [ADR-258](./adr-258-risk-analysis.md) | 风险与收益量化分析 | 📊 参考文档 |
