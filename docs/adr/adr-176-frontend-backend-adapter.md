...
### 数据流追踪
- **🔴 P1 接口覆盖落差**：`wails-bindings.ts:4` 为 `export * from '@bindings/app'` 全量透传（契约测试实测 **139** 函数；注释原称 122、AGENTS 文档原称 116（均已于 2026-07-26 去数字化修正，函数数随契约演进以测试运行时报告为准）），业务 40+ 文件消费具体函数；`BackendService` 仅列 ~10 方法。「改造量小」建立在「接口窄」假设上，实际调用面远宽。**修订**：见文末「调用集实证（2026-07-23 实测）」——已 grep 业务对 139 函数的真实调用集为 **106**（非 ~10），`BackendService` 须覆盖 106 函数或显式降级 17 个原生独占函数；各函数的浏览器降级返回契约（抛错/no-op/null）须逐一定义。
