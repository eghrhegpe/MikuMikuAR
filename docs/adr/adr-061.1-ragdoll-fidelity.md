# ADR-061.1: Ragdoll 保真度补齐（球面关节 + 旋转求解 + 暂停/过渡仲裁 + 关节参数化）

> **状态**: 已交付（2026-07-10）— ①④③ 全部实施完成，14 个 TDD Task 全绿（1351 测试 + tsc 零错误）。③ 方案修订：boneFilter 不可运行时暂停基础 VMD，改用 blendWeight Slerp 混合。 ⚠️ **整篇废弃**（XPBD 移除，见 ADR-081；源码 xpbd-ragdoll/ragdoll-manager 已删）
> **日期**: 2026-07-10
