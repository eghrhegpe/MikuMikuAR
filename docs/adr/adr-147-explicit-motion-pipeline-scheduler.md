...
> **状态**: 实施中（Phase 1 内核切片 + step 2 运行时接入已落地，根治 R1；Phase 2 step 2 运行时接入待协调）
...
> **范围**: 新增 `MotionPipeline` 显式调度器（治理注册时序隐式定序）+ 可选 `BoneOverrideStore` 集中状态（治理三套作用域副本）。Phase 1 已落地（内核 + `scene.ts`/`bone-override.ts`/`perception.ts` 接入，根治 R1）；Phase 2 运行时接入待与 `motion-modules/*` 写者协调。
