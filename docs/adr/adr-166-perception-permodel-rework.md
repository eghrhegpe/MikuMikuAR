...
> **状态**: 已完成（2026-07-21；P4-4 对象池 per-context 追加于 2026-07-22 commit 20f1e8f7；独立审核：tsc 0 错误 / frontend 1829 测试全绿）
...
| ~~P4-4~~ | ~~🟢~~ | ~~perception-shared.ts:89-94~~ | ~~对象池容量~~ | ✅ 已修复（commit 20f1e8f7）：全局单例池 → per-context 独立池（`PerceptionPool`），每模型 32q/16m/8v，溢出 new 不覆写 |
...
- ✅ **已实现**（commit 20f1e8f7）：per-context 独立池（`PerceptionPool`）。每 model 32q/16m/8v，溢出走 `new` 不覆写。`_applyPerceptionForContext` 通过 `_setContextPool(ctx.pool)` + `_resetContextPool()` 按 context 切换当前池。全局 `_currentPool` 为 null 时溢出走 new，零外泄风险。
...
| 对象池 per-context（✅ commit 20f1e8f7） | 代码审查 |
| 现有 1829 测试 + perception 测试全绿 | `npm run test` |
