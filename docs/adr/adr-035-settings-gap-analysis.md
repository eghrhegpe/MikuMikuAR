...
**日期**: 2026-07-05
> **状态**: 已完成 (Completed) — Phase A / B / C 全部落地（含 i18n 语言切换、自动更新开关、默认模型 auto-center），所有设置缺口均已补齐，见下方「实施进度」
...
| **语言切换** | 多语言 | ✅ 已实现（i18n 框架 + 语言设置面板，见「实施进度」） | 非中文用户可切换界面语言 |
...
| **默认模型行为** | 加载缩放/自动居中 | ✅ 已实现（auto-scale + auto-center，性能页开关控制） | 大模型加载后自动归一化并居中取景 |
...
| 自动更新开关 | 检查更新频率 | ✅ 已实现（关于页开关，见「实施进度」） |
...
| 语言切换 (i18n) | P0 | ✅ 已完成（`core/i18n` 框架 + 5 套 locale bundle + `settings-language` 面板 + 运行时 `setLang` 热切换） |
...
| 默认模型行为 | P1 | ✅ 已完成（auto-scale + auto-center 均已实现；auto-center 由性能页「默认模型自动居中」开关控制，`uiState.autoCenterModel`↔`focus(id, frameCamera)` 联动，`arrange()` 多模型排布不受影响）|
...
| 自动更新开关 | P2 | ✅ 已完成（关于页开关 → `SetUIAutoUpdate` → `uiState.autoUpdateEnabled` 持久化，main.ts 启动检查）|
