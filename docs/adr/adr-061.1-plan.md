# ADR-061.1: Ragdoll 保真度补齐 实施计划

> **状态**: 已交付（2026-07-10）— 见 [adr-061.1-ragdoll-fidelity.md](adr-061.1-ragdoll-fidelity.md)；①④③ 全部实施完成，14 个 TDD Task 全绿 ⚠️ **整篇废弃**（XPBD 移除，见 ADR-081）
> **日期**: 2026-07-10

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 实现 ADR-061.1 的 ①④③，把 ragdoll 从 MVP 占位（distance 约束 + Identity 回写 + observer 时序覆盖 + 硬编码参数）升级为真物理布偶（球面关节 + 旋转求解 + blendWeight 混合 + per-joint 参数化）。
...
ADR-061.1 §2.2 原设想"经 Motion Layer 控制（参考 boneFilter 屏蔽机制）暂停其 VMD 动画写入"。代码核实结论：**boneFilter 不可用于此目的**。
...
### Spec coverage（ADR-061.1 验收标准对照）
