...
| **P3 触碰即改善** | ✅ 已完成（2026-07-29）：`perception.test.ts`（1431 行 / 100 用例 / 18 处 `vi.mock`）拆为 8 个 `perception/*.int.test.ts`（均 ≤300 行）；`mockState`/`mockPipeline` 以 `vi.hoisted` 内联留各测试文件（避免 `vi.resetModules()` 驱逐外置模块导致新旧实例脱节），`perception-mocks.ts` 收敛 18 个 `vi.mock` 工厂函数 + `setupPerceptionTest` + 共享 morph 助手 | 用例数守恒（100→100、全量 2437 passed）、单文件 ≤300 行、上帝文件数递减 |
