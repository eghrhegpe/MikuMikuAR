...
**近 30 天 2,160 个 commit 的历史数据分析**：

| 风险场景 | 发生概率 | 影响 | 决策 |
|---------|---------|------|------|
| tsc 类型错误漏到 main | 2% (44 commits) | 高（编译失败） | **P0 阻断（引入 pre-push）** |
| 测试失败漏到 main | 10% (~20 commits) | 高（功能回归） | **P0 阻断（维持）** |
| 覆盖率下降 | 30% (~30 commits) | 中（技术债累积） | **P1 建议（改到 commit 期）** |
| 死代码积累 | 50% (~50 处) | 低（仅影响维护） | **P2 参考（仅 CI 跑）** |
| docs 变更仍跑 TS 检查 | 100% (31% push) | 中（浪费算力） | **分流优化（省 30s）** |

**关键发现**：
- **75% 的 CI 失败是 `check-env-parity` 漂移**（TS schema 与 Go bindings 不同步），但 pre-push 无法拦截（仅 CI 侧运行）
- **docs 变更占 31% 的 push**，当前已支持快速放行（节省 30s × 31% = 168h/年）
- **tsc 修复需 3s**，但 main 分支发现需 30min（20 倍成本差）
...
1. **守住 P0 红线**（不可放松）：
   - `tsc` — 44 个 tsc fix commit 证明是真实风险
   - `vitest --changed` — 测试失败 = 功能回归
   - `i18n --strict` — 多语言用户可见硬伤

2. **优化体验**（已实施）：
   - `lint:changed` 增量扫描 — 16s → 0.3s（-98%）
   - `docs-only` 快速放行 — 节省 30s × 31% push
   - `collect-all` 模式 — 一次性报告所有错误

3. **交给 CI 兜底**（不阻塞 push）：
   - `deadcode` — 技术债可视化，不阻塞开发
   - `test:coverage` 全量 — schedule job 兜底
   - `diff-coverage` — 改到 commit 期护栏
