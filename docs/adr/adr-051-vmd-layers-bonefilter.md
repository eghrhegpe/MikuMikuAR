...
`MmdWasmRuntime` 不支持 `MmdCompositeAnimation` —— 其 `createRuntimeAnimation` 期望 `MmdWasmAnimation`，而 composite 对象不兼容。检测到 WASM 运行时：

1. 打印 `console.warn` + 状态栏提示"WASM 仅支持单图层"
2. 只加载第一个图层（或基础 VMD）到单 VMD 管线
3. 不影响应用运行，图层 UI 和配置保持不变

JS 运行时（`VITE_MMD_RUNTIME=js`）完整支持图层混合。
