...
> **修订注记（2026-07-13）**: 运动语义于 **ADR-100** 升级为「控制方案 × 运动行为」双轴。本文档的 `concert`（扫掠+上下摆动）与 `surround`（整圈自转/转台）公式仍为**权威实现来源**，二者现分别成为双轴行为轴的 `concert` / `turntable` 行为；原文「`surround` 不纳入自动运镜」等特例排除逻辑已在 ADR-100 中删除（行为互斥，无需硬编码排除）。
