...
- **`e2e-wails`（best-effort，windows-latest，`continue-on-error`）**：跑 `@webgl` 标签的 spec（model-load / action-play / export-screenshot / env-sky 截图），`wails3 dev` 启动真实 WebView2 并开放 CDP 9222，Playwright `connectOverCDP` 连接断言 3D 渲染（`window.__scene` 数值钩子 + 截图基线）。
