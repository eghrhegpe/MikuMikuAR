...
- **P1.2 per-model 守卫**：`loading.extraction` 从 `boolean` 升级为 `Set<string>`（按模型 id 守卫），非 zip 模型直接放行 ✅ 已完成（P0.1 实施时 store API 已为 Set 签名，P0.3 前调用方已传 per-model key；grep 确认无 `isExtracting()` 无参调用残留）
- **P1.3 ADR-131 后续清理**：3 个绑定标志位（`motionBindingTargetId` / `layerBindingTargetId` / `modelReplaceTargetId`）从 state.ts 移到 store 的 `binding` 命名空间，最终由 ADR-131 契约彻底移除 ✅ 已完成（ADR-131 实现时已通过 BrowseOutcome 统一契约彻底移除，grep 确认无变量定义/调用残留，仅存 2 处注释引用）
