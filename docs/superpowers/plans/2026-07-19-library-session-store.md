...
| P3 | PopupRow discriminated union | ⏸ 评估后暂不实施（见第 16 节） | 评估文档 |
...
## 16. P3 PopupRow discriminated union — 评估后暂不实施（2026-07-20）

### 16.1 评估背景

P3 是 plan 原定的最后一个阶段：将 `PopupRow` 从单一宽松 type 改造为 discriminated union，让 TypeScript narrowing 生效，编译期捕获「跨 kind 字段访问」错误。

### 16.2 调研结果

**构造点分布**（`grep kind: 'xxx'` 统计）：

| 文件 | 构造点数 | 备注 |
|------|---------|------|
| `env-feature-levels.ts` | 100 | 但实际是 MenuNode schema（有 `control` 字段），非 PopupRow 直接构造 |
| `__tests__/menu.test.ts` | 48 | 测试用例 |
| `scene-render-levels.ts` | 27 | 同上，MenuNode |
| `motion-gaze-levels.ts` | 18 | MenuNode |
| `env-menu.ts` | 15 | PopupRow 直接构造 |
| `motion-popup.ts` | 14 | PopupRow |
| `scene-menu.ts` | 13 | PopupRow |
| `hand-symmetry.ts` | 13 | MenuNode |
| `settings.ts` / `settings-paths.ts` | 10/10 | PopupRow |
| `library-core.ts` | 10 | PopupRow |
| 其他 | ~10 | 混合 |

**真实 PopupRow 直接构造点约 ~100 处**，分布 12+ 文件。MenuNode 构造点（348 总数）不在 P3 范围。

### 16.3 改造方案评估

#### 方案 A：严格 discriminated union（每 kind 必填字段）

- 价值：编译期捕获「遗漏必填字段」错误
- 风险：~100 处构造点需逐一检查并补齐 sliderMin/Max/Step 等字段
- 评估：**风险过高**。很多构造点可能省略了应必填字段，逐一补齐工作量大且易引入回归

#### 方案 B：宽松 discriminated union（字段保持 optional，按 kind 分组）

- 价值：编译期捕获「跨 kind 字段访问」错误（如 slider 行访问 toggleValue）
- 风险：消费方代码报错
- 评估：**仍有问题**。例如 [menu.ts:731-751 refreshRowText](file:///c:/Users/zhujieling11/MikuMikuAR/frontend/src/menus/menu.ts#L731-L751) 在 `if (slider || toggle || modeSlider || chips) return;` 后访问 `row.sublabel` 和 `row.model`，但 divider/sectionTitle 类型无这些字段 → tsc 报错

#### 方案 C：helper function 封装（createSliderRow / createToggleRow 等）

- 价值：helper 内部强制必填字段，构造点逐步迁移
- 风险：需新增 helper 文件 + 逐步迁移构造点，工作量分散
- 评估：**可行但工作量大**，且不属于 P3 原定范围

### 16.4 决策：暂不实施

**理由**：

1. **当前代码运行时稳定**：消费方已用 `?? 0` / `?? false` 兜底，不会因字段缺失崩溃
2. **改造成本高**：~100 处构造点 + 消费方报错修复，风险/收益比不划算
3. **AGENTS.md 原则**：「避免高难度维护」— 348 处改动属于高难度维护
4. **替代方案存在**：未来可用 helper function 方案逐步迁移，风险可控
...
| 触发条件 | 说明 |
|---------|------|
| 新增 PopupRow kind 时 | 此时改造可顺便整理现有结构 |
| 出现「跨 kind 字段访问」导致的运行时 bug | 证明 discriminated union 有实际价值 |
| 构造点重构时 | 顺便迁移到 helper function 方案 |
| PopupRow 字段数超过 30 个 | 类型过于宽松，必须用 union 收敛 |

### 16.6 替代缓解措施

在 P3 暂不实施期间，采取以下缓解措施：

| 措施 | 说明 |
|------|------|
| 构造点用对象字面量 + `as const` 标注 kind | 让 kind 字面量类型生效，部分 narrowing 受益 |
| 消费方用 `?? 默认值` 兜底 | 已实施，运行时安全 |
| 新增 kind 时同步更新 [types.ts:306-358](file:///c:/Users/zhujieling11/MikuMikuAR/frontend/src/core/types.ts#L306-L358) 注释 | 保持文档与代码一致 |
| 新增 kind 时在 [menu.ts createRow](file:///c:/Users/zhujieling11/MikuMikuAR/frontend/src/menus/menu.ts#L892-L960) 显式分支处理 | 避免 fallthrough 到错误分支 |
