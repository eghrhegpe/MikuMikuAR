...
- `tryAttachGizmoFromPick(scene, x, y)` — 拾取命中后挂 Gizmo；**attach 成功即 `setSelectedTransformTarget(result)` 同步选中态**，使面板滑块/删除/重试等基于选中态的逻辑与场景点击一致（P2 修复）
...
- `tryAttachGizmoFromPick(scene, x, y): boolean`

## 关键约定
- 元数据挂在父节点上即可让全部子 mesh 继承，避免逐 mesh 标注
- 拾取过滤保证只有「可变换资源」能被 Gizmo 选中
- 场景点击与面板选中共用同一 `transform-selection` 选中态；`syncDragMode` 内已有「`getGizmoTargetId() === _selected.id` 跳过重复 attach」守卫，故场景点击后 setSelected 不会 detach+重建闪烁
...
- 依赖 `transform-selection.ts`（`setSelectedTransformTarget`，同向无循环）
- 下游：Gizmo 拖拽选中逻辑经此处识别目标 kind/id；场景拖拽入口在 `scene.ts` `_bindSceneEvents`（ADR-171，POINTERUP 位移<5px 调用）
