...
graph TD;
    ...
    subgraph cat_core["核心基础设施"]
        c72["统一动作注册表 — 菜单/NL/快捷键共享真相源"]
        c73["AI 配置持久化（IndexedDB）"]
        c74["错误环形缓冲与全局捕获"]
        c75["NL 意图解析 — LLM 文本 → 动作执行"]
        c76["场景运行时快照（AI 上下文）"]
        c77["内置 AI 诊断助手 — 双适配器服务层"]
        c78["安卓文件访问（shared 模式）"]
        c79["音频总线"]
        c80["后端适配层"]
        c81["EnvState 默认值派生"]
        c82["EnvState 单一源 Schema"]
        c83["事件处理与导航系统"]
        c84["结构化反馈 API"]
        c85["统一文件服务层"]
        c86["应用启动引导"]
        c87["统一资源加载队列"]
        c88["模型加载/库扫描完成后菜单刷新注册表"]
        c89["国际化语言状态"]
        c90["babylon-mmd 适配边界"]
        c91["轨道相机键盘输入状态叶子"]
        c92["渲染循环与 FPS 时钟"]
        c93["资源加载失败统一汇总"]
        c94["Runtime 隔离桥（Wails Events/Browser）"]
        c95["运行模式检测"]
        c96["快捷键注册表"]
        c97["全局状态与场景运行时 Store"]
        c98["键盘导航工具"]
        c99["后端绑定聚合层（backend 代理化）"]
    end

    subgraph cat_backend["后端"]
        c100["Go 后端核心（App 生命周期 + 配置系统）"]
        c101["Go 文件与路径平台抽象"]
        c102["Go 模型隔离与安全 HTTP"]
        c103["Go 软件集成（Blender/MMD/自定义）"]
        c104["Go KTX2 纹理编码"]
        c105["Go 模型库扫描"]
        c106["Go LLM 客户端与 AI 绑定"]
        c107["Go 广场窗口与配置"]
        c108["Go 预设持久化与标签"]
        c109["Go 模型广场代理（SSRF 防护）"]
        c110["Go 场景序列化与打包"]
        c111["Go 更新检查与安装"]
        c112["Go 下载目录监听与导入"]
        c113["Go ZIP 解压与缓存管理"]
