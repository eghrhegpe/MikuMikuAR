...
graph TD;
    ...
    subgraph cat_core["核心基础设施"]
        ...
        c82["后端适配层"]
        c83["EnvState 默认值派生"]
        c84["EnvState 单一源 Schema"]
        c85["事件处理与导航系统"]
        c86["结构化反馈 API"]
        c87["统一文件服务层"]
        c88["应用启动引导"]
        c89["统一资源加载队列"]
        c90["模型加载/库扫描完成后菜单刷新注册表"]
        c91["国际化语言状态"]
        c92["babylon-mmd 适配边界"]
        c93["轨道相机键盘输入状态叶子"]
        c94["渲染循环与 FPS 时钟"]
        c95["资源加载失败统一汇总"]
        c96["Runtime 隔离桥（Wails Events/Browser）"]
        c97["运行模式检测"]
        c98["快捷键注册表"]
        c99["全局状态与场景运行时 Store"]
        c100["键盘导航工具"]
        c101["后端绑定聚合层（backend 代理化）"]
    end

    subgraph cat_backend["后端"]
        c102["Go 后端核心（App 生命周期 + 配置系统）"]
        c103["Go 文件与路径平台抽象"]
        c104["Go 模型隔离与安全 HTTP"]
        c105["Go 软件集成（Blender/MMD/自定义）"]
        c106["Go KTX2 纹理编码"]
        c107["Go 模型库扫描"]
        c108["Go LLM 客户端与 AI 绑定"]
        c109["Go 广场窗口与配置"]
        c110["Go 预设持久化与标签"]
        c111["Go 模型广场代理（SSRF 防护）"]
        c112["Go 场景序列化与打包"]
        c113["Go 更新检查与安装"]
        c114["Go 下载目录监听与导入"]
        c115["Go ZIP 解压与缓存管理"]
    ...
    click a178 href "../adr/adr-178-capability-matrix-host-keys.md"
