...
> 能力键定义见 `frontend/src/core/backend/types.ts` 的 `BackendCapabilities`；后端原生键 13 个（ADR-176），宿主运行时键 3 个（ADR-178，Proposed）。
...
| `arScope` | `none` | `android-app` | `none` | AR 作用域：无 / 安卓应用 ARCore / 网页 WebXR（未接通，待立） |
