...
> **源码仓库**: https://github.com/noname0310/babylon-mmd
...
#### `MmdWasmInstanceTypeMPR` — 多线程物理

**模块**: `Runtime/Optimized/InstanceType/multiPhysicsRelease`

多线程 + 物理 Release 版本。当前项目使用 `MmdWasmInstanceTypeSPR`（单线程）。
...
**搬运收益**: 多核设备上物理+IK 求解并行化，性能显著提升。一行替换 `MmdWasmInstanceTypeSPR` → `MmdWasmInstanceTypeMPR`。

**注意事项**: 多线程需要 `SharedArrayBuffer`，需要 COOP/COEP HTTP 头。Wails 内置 WebView 可能需要额外配置。

---

#### `MmdOutlineRenderer` — 描边渲染
...
#### `MmdCompositeRuntimeModelAnimation` — JS 运行时动画混合
...
| 优先级 | 项目 | 工作量 | 收益 |
|--------|------|--------|------|
| **P0** | `MmdWasmInstanceTypeMPR` | 极小（一行替换） | 多线程性能提升 |
| **P0** | 向上游提交接口补全 PR | 中（需 fork/PR） | 消除所有 `as any` |
| **P1** | `StreamAudioPlayer` | 中（替换音频管线） | 音画同步简化 |
| **P1** | `AnimationRetargeter` + `HumanoidMmd` | 中（新增导入路径） | 解锁 Mixamo/VRM 动作 |
| **P2** | `MmdOutlineRenderer` | 小（side-effect import） | MMD 描边效果 |
| **P2** | `MmdCompositeRuntimeModelAnimation` | 小 | JS runtime 动画混合简化 |
| **P3** | `BpmxConverter` / `BvmdConverter` | 大（需构建流程） | 加速加载 |
| **P3** | `PBRMaterialBuilder` | 中（材质系统改造） | PBR 渲染 |
| **P3** | `SdefInjector` / `SdefMesh` | 小 | SDEF 变形效果 |
...
babylon-mmd v1.2.0 提供了 ~131 个导出 API，项目直接使用了约 39 个。主要未利用的高价值功能集中在：

1. **音频子系统** (`StreamAudioPlayer`) — 与自建管线的取舍
2. **人形重定向** (`AnimationRetargeter` / `HumanoidMmd`) — 扩展动作数据来源
3. **多线程 WASM** (`MPR` 实例类型) — 性能提升
4. **描边渲染** (`MmdOutlineRenderer`) — 视觉效果

类型安全方面，`IMmdModel` 接口缺少动画绑定方法是最大的系统性问题，建议向上游提交补全 PR。

---

## 八、派生 ADR（草案 · 待评审）

> 以下 ADR 由本调研推导，编号 **063–068**，状态均为「草案」。评审通过后应提升至 `docs/adr/` 并按规范补元数据（关联 ADR、验证记录、审核记录）。编号承接现有 ADR 序列（已至 ADR-062）。

---

# ADR-063: 多线程 WASM 物理实例迁移（MmdWasmInstanceTypeMPR）

> **状态**: 草案 · 待评审
> **日期**: 2026-07-08
> **来源**: 本调研 §2.1 / §五 P0
> **关联**: ADR-056（Motion Layers）、Wails v3 文件服务器机制（memory）
...
# ADR-064: 向上游补全 IMmdModel 接口以消除类型安全缺口

> **状态**: 草案 · 待评审
> **日期**: 2026-07-08
> **来源**: 本调研 §3.1 / §五 P0
> **关联**: ADR-072（MmdCompositeRuntimeModelAnimation，可消解一处 cast）
...
- 🔗 `vmd-layers.ts:577` 的 cast 在 ADR-072（`MmdCompositeRuntimeModelAnimation`）引入后可能自然消解

---

# ADR-069: 引入 StreamAudioPlayer 替代自建音频管线

> **状态**: 草案 · 待评审
> **日期**: 2026-07-08
> **来源**: 本调研 §2.1 StreamAudioPlayer / §五 P1
> **关联**: ADR-021（BeatDetector / 程序化动作）、`outfit/audio.ts` 现状
...
# ADR-070: 引入 AnimationRetargeter + HumanoidMmd 扩展动作来源

> **状态**: 草案 · 待评审
> **日期**: 2026-07-08
> **来源**: 本调研 §2.1 AnimationRetargeter / HumanoidMmd / §五 P1
> **关联**: ADR-061（高级骨骼 / Accessory / T-pose，同源骨骼名映射需求）
...
# ADR-071: 引入 MmdOutlineRenderer 描边渲染
...
# ADR-072: 引入 MmdCompositeRuntimeModelAnimation 简化 JS 动画混合

> **状态**: 草案 · 待评审
> **日期**: 2026-07-08
> **来源**: 本调研 §2.1 MmdCompositeRuntimeModelAnimation / §五 P2
> **关联**: ADR-056（Motion Layers）、ADR-064（类型缺口消解）
...
> **草案小结**：P0（063 MPR / 064 类型补全）应优先评审；P1（065 音频 / 066 重定向）涉及管线重构，建议排期在中；P2（067 描边 / 068 JS 混合）侵入小，可随时落地。全部评审通过后，本文件中 6 篇 ADR 应提升至 `docs/adr/adr-06[3-8]-*.md` 并补全元数据。
