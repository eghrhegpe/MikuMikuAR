...
└── frontend/
    └── src/
        ...
        ├── library.ts      # 模型库弹窗 + 搜索 + MenuStack
        ├── settings.ts     # 设置页 + 外部库管理
        ├── menu.ts         # MenuStack 通用导航栈
        ├── camera.ts       # 相机模式管理（轨道/自由飞行/演唱会）
        ├── fileservice.ts  # 统一文件 URL 解析层
        ├── scene-menu.ts  # 场景弹窗（相机/灯光/渲染预设）
...
scene.ts
 ├── config.ts        状态读写、工具函数
 ├── camera.ts        autoFrame / getCameraState / setCameraState
...
scene-menu.ts
 ├── config.ts        状态、类型
 ├── menu.ts          MenuStack
 ├── camera.ts        switchCameraMode / getCameraMode / getLightState / setLightState
 ├── scene.ts         triggerAutoSave / serializeScene / deserializeScene / getRenderState / setRenderState
 └── Wails Binding    SelectSceneSaveFile / SaveSceneFile / ...
