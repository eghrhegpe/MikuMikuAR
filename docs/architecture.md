...
### 14. 相机 VMD（`scene/camera/camera.ts` + `scene/scene.ts`）
...
### 15. 舞蹈套装（`menus/library.ts` + `internal/app/dancesets.go`）
...
### 10.1 视线追踪子系统（`scene/motion/proc-motion-bridge.ts`）
...
| `core/config.ts:OutfitFile/OutfitVariant/OutfitSlot` | 类型定义 |
