...
export class XpbdRenderer {
  ...
  updateParticles(solver: XpbdSolver): void {
    const BABYLON = (globalThis as any).BABYLON;
    if (!BABYLON || !BABYLON.MeshBuilder) return;
  ...
  updateConstraints(solver: XpbdSolver): void {
    ...
    if (!this.constraintVisible) return;

    const positions: number[] = [];

    for (const c of solver.constraints) {
      if (c.type === "volume") continue; // 四面体不画线
  ...
  dispose(): void {
    this._clearConstraintLines();
    for (const m of this.particleMeshes) m?.dispose?.();
    for (const m of this.colliderMeshes) m?.dispose?.();
    this.particleMeshes = [];
    this.colliderMeshes = [];
