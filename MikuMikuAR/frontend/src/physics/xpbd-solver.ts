...
export class XpbdSolver {
  ...
  removeParticle(idx: number): void {
    ...
    if (p.invMass === 0) return; // 已经是固定/已删除
    p.invMass = 0;
    // 清除关联约束
    this.constraints = this.constraints.filter(
      (c) => !c.indices.includes(idx),
    );
  ...
  addGroundCollision(groundY?: number): void {
    if (groundY !== undefined) this.groundY = groundY;
  ...
  /** 距离/弯曲约束求解 */
  private _solveDistanceConstraint(
    c: XpbdConstraint,
    alphaTilde: number,
  ): void {
  ...
  /** 体积约束求解（四面体） */
  private _solveVolumeConstraint(
    c: XpbdConstraint,
    alphaTilde: number,
  ): void {
  ...
  private _tetraVolumeGradients(
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const e1x = pj[0] - pk[0], e1y = pj[1] - pk[1], e1z = pj[2] - pk[2];
      const e2x = pl[0] - pk[0], e2y = pl[1] - pk[1], e2z = pl[2] - pk[2];
