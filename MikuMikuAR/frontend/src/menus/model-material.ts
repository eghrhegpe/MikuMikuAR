    ...
    renderCustom: (container) => {
      ...
      cardContainer(container, (c) => {
        if (overrideCount > 0) {
          ...
          hint.style.cssText = "font-size:11px;color:var(--warn);margin-bottom:8px;padding:4px 10px;background:var(--white-04);border-radius:6px;text-align:center;";
        ...
        for (const [cat, mats] of groups) {
          ...
          addCollapsible(c, {
            ...
            defaultOpen: false,
            renderContent: (panel) => {
              addSliderRow(panel, "漫反射倍率", params.diffuseMul, 0, 2, 0.05, (v) => setMatCatParams(id, cat, { diffuseMul: v }));
              addSliderRow(panel, "高光倍率", params.specularMul, 0, 2, 0.05, (v) => setMatCatParams(id, cat, { specularMul: v }));
              addSliderRow(panel, "高光指数", params.shininess, 0, 200, 1, (v) => setMatCatParams(id, cat, { shininess: v }));
              addSliderRow(panel, "环境光倍率", params.ambientMul, 0, 2, 0.05, (v) => setMatCatParams(id, cat, { ambientMul: v }));
            },
          });
        }
      });
    ...
    renderCustom: (container) => {
      ...
      cardContainer(container, (c) => {
        if (groups.size === 0) {
          ...
          empty.style.cssText = "padding:12px 14px;text-align:center;font-size:11px;color:var(--text-dim);";
          ...
          addCollapsible(c, {
            ...
            renderContent: (inner) => {
              for (const matInfo of mats) {
                ...
                if (idx === -1) continue;
                const isSelected = _selectedMat?.cat === cat && _selectedMat?.index === idx;
      ...
      cardContainer(container, (c) => {
        if (_selectedMat) {
          ...
          const resetRow = document.createElement("div");
          resetRow.className = "slide-item";
          resetRow.innerHTML = `<span class="slide-icon"><iconify-icon icon="lucide:rotate-ccw"></iconify-icon></span><span class="slide-label">重置此材质</span>`;
          resetRow.addEventListener("click", () => {
          ...
          const hint = document.createElement("div");
          hint.style.cssText = "font-size:11px;color:var(--text-dim);padding:12px 14px;text-align:center;";
      ...
      cardContainer(container, (c) => {
        ...
        resetAllRow.innerHTML = `<span class="slide-icon"><iconify-icon icon="lucide:refresh-ccw"></iconify-icon></span><span class="slide-label">重置全部材质参数</span>`;
