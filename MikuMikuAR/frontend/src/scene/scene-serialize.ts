...
import {
    ...
    envState, EnvState,
    modelRegistry, propRegistry,
} from "../core/config";
import {
    getCameraState, setCameraState,
    hasCameraVmd, getCameraVmdPath, getCameraVmdName, getCameraMode,
    switchCameraMode,
} from "./camera";
import { loadCameraVmdFromPath } from "./scene-vmd";
import type { CameraState } from "./camera";
import {
    getAudioName, getAudioPath, getVolume, getAudioOffset, isAudioPlaying,
    loadAudioFile, setVolume, setAudioOffset, resumeAudio,
} from "../outfit/audio";
import { loadOutfits, applyOutfitVariant } from "../outfit/outfit";

import {
    getLightState, setLightState,
    getRenderState, setRenderState,
    removeModel, loadPMXFile, focusedModel,
    setModelBoneLinesVis, setModelBoneJointsVis, setModelPhysics,
    loadVMDFromPath,
    LightState, RenderState,
} from "./scene";
...
export async function deserializeScene(data: SceneFile, skipEnv = false): Promise<void> {
    ...
    if (data.lipSync) {
        setLipSyncState({ ...DEFAULT_LIPSYNC_STATE, ...data.lipSync as Partial<LipSyncStateType> });
    } else {
