...
export function getRenderState(): RenderState {
    if (!_scene || !pipeline) return _defaultRenderState();
...
export function setRenderState(s: Partial<RenderState>): void {
    if (!pipeline || !_scene || !_modelRegistry || !_triggerAutoSave) return;
...
export function rebuildOutlineState(): void {
    if (!_modelRegistry) return;
    for (const inst of _modelRegistry.values()) {
        for (const m of inst.meshes) {
            if (_outlineEnabled) {
                m.enableEdgesRendering();
                if (m.edgesRenderer) {
                    m.edgesColor = new Color4(_outlineColor[0], _outlineColor[1], _outlineColor[2], 1);
                }
            } else {
                m.disableEdgesRendering();
            }
