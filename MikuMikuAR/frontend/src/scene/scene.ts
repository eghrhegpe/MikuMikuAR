...
import { initEnvFacade, applyEnvState, _envSys, refreshWaterRenderList, addRipple } from "./scene-env";
...
import {
    dom, setStatus, formatTime,
    setMmdRuntime, mmdRuntime, modelRegistry, focusedModelId, setFocusedModelId,
    isPlaying, setIsPlaying, autoLoop,
    isLoadingModel, setIsLoadingModel, isLoadingVmd, setIsLoadingVmd, setAutoLoop,
    seekDragging, setSeekDragging,
    ModelInstance, setModelRegistry,
    PropInstance, propRegistry,
    envState, EnvState,
    triggerAutoSave, setTriggerAutoSave,
    pendingVmd, setPendingVmd,
} from "../core/config";
import { resolveFileUrl, normPath } from "../core/fileservice";
import { syncAudioPlayback, attachBeatDetector, disposeAudio, isAudioPlaying } from "../outfit/audio";
...
import { initLighting, getLightState, setLightState, _updateSunDisc, rebuildShadowCasters, type LightState } from "./scene-lighting";
import { initRenderer, getRenderState, setRenderState, reattachPipeline, rebuildOutlineState, pipeline, type RenderState } from "./scene-renderer";
...
import { updateProcMotion, stopProcMotion, procVmdActive, createProcBeatDetector, getProcBeatDetector } from "./scene-proc-motion";
...
export async function initScene(): Promise<void> {
    ...
    initPlaybackObservables(runtime, updatePlaybackUI, updateProcMotion, updateLipSync, getProcBeatDetector);
