...
function buildWaterLevel(): PopupLevel {
    return {
        label: "水面",
        dir: "",
        items: [],
        renderCustom: (container) => {
            ...
            addSliderRow(container, "水位高度", s.waterLevel, -5, 5, 0.1, (v) => setEnvState({ waterLevel: v }), "lucide:move-vertical");
            ...
            addSliderRow(container, "波浪强度", s.waterWaveHeight, 0, 2, 0.05, (v) => setEnvState({ waterWaveHeight: v }), "lucide:waves");
            addSliderRow(container, "水面尺寸", s.waterSize, 10, 200, 5, (v) => setEnvState({ waterSize: v }), "lucide:maximize");
        },
    };
}
